/*
  Freshly Mart Apps Script Backend V9 - Cancellation + Track Order + WhatsApp Notifications
...
const FM_VERSION = '9.0-cancellation-track-whatsapp';
...
const FM_STATUS = {
  ...
  ORDER: ['New','Confirmed','Procurement Pending','Procured','Sent to Hub','Ready for Pickup','Out for Delivery','Delivered','Cancellation Requested','Cancellation Approved','Cancellation Rejected','Cancelled','Refunded'],
  PAYMENT: ['UPI Pending','UPI Pending Verification','UPI Pending at Delivery','UPI Pending at Hub','Paid to Freshly Mart','Payment Failed','Cancelled - No Payment','Refund Pending','Refund Initiated','Refund Completed'],
  ...
  CANCELLATION: ['Cancellation Requested','Cancellation Approved','Cancellation Rejected','Cancelled','Refund Pending','Refund Initiated','Refund Completed'],
...
const FM_SHEETS = {
  ...
  Customers: ['CustomerID','DateTime','CustomerName','Phone','WhatsApp','Email','Address','Area','Pincode','PreferredHubID','PreferredHubName','CurrentLatitude','CurrentLongitude','LocationMapURL','LoginCode','Status','LastLogin','Remarks'],
  ...
  Orders: ['OrderID','DateTime','CustomerID','CustomerName','Phone','WhatsApp','Address','Area','Pincode','HubID','HubName','CurrentLatitude','CurrentLongitude','LocationMapURL','Items','ProductTotal','DeliveryCharge','GrandTotal','PaymentMode','PaymentStatus','OrderStatus','DeliveryOption','DeliverySlot','SellerIDs','SupplierIDs','Notes'],
  ...
  Refunds: ['RefundID','DateTime','ReturnID','OrderID','CustomerName','RefundAmount','RefundMode','RefundStatus','RefundDate','UPIReference','RefundReason','Remarks'],
  Cancellations: ['CancellationID','DateTime','OrderID','CustomerName','Phone','Reason','PaymentStatus','OrderStatusAtRequest','RefundRequired','RefundAmount','RefundUPIID','RequestDate','CancellationStatus','ApprovedBy','ApprovedDate','AdminRemarks','WhatsAppLink','SourcePage'],
  ...
  OrderStatusHistory: ['HistoryID','DateTime','OrderID','OldStatus','NewStatus','PaymentStatus','UpdatedBy','NotifyCustomer','Remarks'],
  WhatsAppNotifications: ['NotificationID','DateTime','OrderID','Phone','CustomerName','NotificationType','Message','WhatsAppLink','SendMode','SendStatus','SentAt','Remarks'],
...
function onOpen(){
  ...
  ui.createMenu('Freshly Mart Backend')
    ...
    .addSubMenu(ui.createMenu('Payments & Receipts')
      ...
      .addItem('Update Order Status / WhatsApp', 'updateOrderStatusPrompt')
      .addItem('Approve / Reject Cancellation by Order ID', 'reviewCancellationPrompt')
...
function seedSettings_(){
  ...
  const defaults = [
    ...
    ['CustomerPortal','Enabled','Active'],
    ['WhatsAppNotificationMode','Link','Active'],
    ['WhatsAppCloudPhoneNumberID','','Active'],
    ['WhatsAppCloudToken','','Active'],
    ['WhatsAppTemplateLanguage','en','Active'],
...
function setupFreshlyMartBanners(showAlert){
  ...
  const rows = [
    ...
    ['BNR004','Sell on Freshly Mart','Sellers can onboard, submit products and go live only after Freshly Mart approval.','','Sell With Us','sell-with-us.html','Sell With Us','sell-with-us.html','Hero',4,'Active','#1e3a8a','#2563eb','#ffffff','🏪'],
...
function doGet(e){
  try{
    ...
    if(action === 'trackOrder') return json_(trackOrder_(e.parameter));
    if(action === 'customerOrders') return json_(customerOrders_(e.parameter));
    if(action === 'customerProfile') return json_(customerProfile_(e.parameter));
...
function doPost(e){
  try{
    ...
    const routes = {
      ...
      saveCustomer: saveCustomer_, customerLogin: customerLogin_, updateCustomer: updateCustomer_,
      ...
      updateOrderStatus: function(d){ requireAdmin_(d.AdminToken); return updateOrderStatus_(d); },
      requestCancellation: requestCancellation_,
      approveCancellation: function(d){ requireAdmin_(d.AdminToken); return approveCancellation_(d); },
      rejectCancellation: function(d){ requireAdmin_(d.AdminToken); return rejectCancellation_(d); },
      sendWhatsAppNotification: function(d){ requireAdmin_(d.AdminToken); return sendWhatsAppNotificationAction_(d); },
...
function findCustomerByPhone_(phone){
  const cp = cleanPhone_(phone);
  if(!cp) return null;
  return rowsAsObjects_('Customers').find(function(c){ return cleanPhone_(c.Phone) === cp; }) || null;
}

function publicCustomer_(c){
  c = c || {};
  return {
    CustomerID:c.CustomerID || '', CustomerName:c.CustomerName || '', Phone:c.Phone || '', WhatsApp:c.WhatsApp || '', Email:c.Email || '',
    Address:c.Address || '', Area:c.Area || '', Pincode:normalizePincode_(c.Pincode), PreferredHubID:c.PreferredHubID || '', PreferredHubName:c.PreferredHubName || '',
    CurrentLatitude:c.CurrentLatitude || '', CurrentLongitude:c.CurrentLongitude || '', LocationMapURL:c.LocationMapURL || '', LoginCode:c.LoginCode || '', Status:c.Status || ''
  };
}
...
function saveCustomer_(d){
  ...
  validateRequired_(d, ['CustomerName','Phone','Pincode'], 'Customer signup');
  validatePhone_(d.Phone, 'Phone');
  validatePincode_(d.Pincode);
  const existing = findCustomerByPhone_(d.Phone);
  if(existing){
    updateRowById_('Customers','CustomerID',existing.CustomerID, {
      CustomerName:d.CustomerName || existing.CustomerName, WhatsApp:d.WhatsApp || existing.WhatsApp, Email:d.Email || existing.Email,
      Address:d.Address || existing.Address, Area:d.Area || existing.Area, Pincode:d.Pincode || existing.Pincode,
      PreferredHubID:d.PreferredHubID || existing.PreferredHubID, PreferredHubName:d.PreferredHubName || existing.PreferredHubName,
      CurrentLatitude:d.CurrentLatitude || existing.CurrentLatitude, CurrentLongitude:d.CurrentLongitude || existing.CurrentLongitude, LocationMapURL:d.LocationMapURL || existing.LocationMapURL,
      Status:'Active'
    });
    const updated = findRowObject_('Customers','CustomerID',existing.CustomerID) || existing;
    return {ok:true,message:'Customer account updated. Use your existing login code to login.', CustomerID:existing.CustomerID, LoginCode:updated.LoginCode, customer:publicCustomer_(updated)};
  }
  const id = 'CUS' + Utilities.formatDate(new Date(), FM_TIMEZONE, 'yyMMddHHmmss');
  const code = String(cleanPhone_(d.Phone)).slice(-4) + Utilities.formatDate(new Date(), FM_TIMEZONE, 'ss');
  const row = Object.assign({}, d, {CustomerID:id, DateTime:new Date(), LoginCode:code, Status:'Active'});
  appendObject_('Customers', row);
  logActivity_('saveCustomer','Public','Customer account '+id);
  return {ok:true,message:'Customer account created. Save this login code for future login.', CustomerID:id, LoginCode:code, customer:publicCustomer_(row)};
}

function customerLogin_(d){
  d = d || {};
  validateRequired_(d, ['Phone','LoginCode'], 'Customer login');
  validatePhone_(d.Phone, 'Phone');
  const c = findCustomerByPhone_(d.Phone);
  if(!c) throw new Error('Customer account not found. Please signup first.');
  if(String(c.LoginCode || '').trim() !== String(d.LoginCode || '').trim()) throw new Error('Invalid login code.');
  updateRowById_('Customers','CustomerID',c.CustomerID,{LastLogin:new Date(), Status:'Active'});
  return {ok:true,message:'Customer login successful.', customer:publicCustomer_(findRowObject_('Customers','CustomerID',c.CustomerID) || c)};
}

function updateCustomer_(d){
  d = d || {};
  validateRequired_(d, ['Phone'], 'Customer profile update');
  validatePhone_(d.Phone, 'Phone');
  if(d.Pincode) validatePincode_(normalizePincode_(d.Pincode));
  let c = d.CustomerID ? findRowObject_('Customers','CustomerID',d.CustomerID) : null;
  if(!c) c = findCustomerByPhone_(d.Phone);
  if(!c) return saveCustomer_(d);
  const updates = {
    CustomerName:d.CustomerName || c.CustomerName, WhatsApp:d.WhatsApp || c.WhatsApp, Email:d.Email || c.Email,
    Address:d.Address || c.Address, Area:d.Area || c.Area, Pincode:normalizePincode_(d.Pincode || c.Pincode),
    PreferredHubID:d.PreferredHubID || c.PreferredHubID, PreferredHubName:d.PreferredHubName || c.PreferredHubName,
    CurrentLatitude:d.CurrentLatitude || c.CurrentLatitude, CurrentLongitude:d.CurrentLongitude || c.CurrentLongitude, LocationMapURL:d.LocationMapURL || c.LocationMapURL,
    Status:'Active'
  };
  updateRowById_('Customers','CustomerID',c.CustomerID,updates);
  return {ok:true,message:'Customer profile updated.', CustomerID:c.CustomerID, customer:publicCustomer_(findRowObject_('Customers','CustomerID',c.CustomerID) || c)};
}

function customerProfile_(params){
  params = params || {};
  let c = params.CustomerID ? findRowObject_('Customers','CustomerID',params.CustomerID) : null;
  if(!c && params.Phone) c = findCustomerByPhone_(params.Phone);
  if(!c) return {ok:false,message:'Customer not found.'};
  if(params.LoginCode && String(c.LoginCode || '').trim() !== String(params.LoginCode || '').trim()) return {ok:false,message:'Invalid customer login code.'};
  return {ok:true,customer:publicCustomer_(c)};
}

function customerOrders_(params){
  params = params || {};
  const phone = cleanPhone_(params.Phone || params.phone || '');
  let customerId = String(params.CustomerID || params.customerId || '').trim();
  let c = customerId ? findRowObject_('Customers','CustomerID',customerId) : null;
  if(!c && phone) c = findCustomerByPhone_(phone);
  if(c){ customerId = c.CustomerID; }
  if(c && params.LoginCode && String(c.LoginCode || '').trim() !== String(params.LoginCode || '').trim()) return {ok:false,message:'Invalid customer login code.'};
  const orders = rowsAsObjects_('Orders').filter(function(o){
    return (customerId && String(o.CustomerID || '') === customerId) || (phone && cleanPhone_(o.Phone) === phone);
  }).slice(-20).reverse();
...
function publicOrder_(o){
  o = o || {};
  return {
    OrderID:o.OrderID || '', DateTime:o.DateTime || '', CustomerID:o.CustomerID || '', CustomerName:o.CustomerName || '', Phone:o.Phone || '', WhatsApp:o.WhatsApp || '',
    Address:o.Address || '', Area:o.Area || '', Pincode:o.Pincode || '', HubID:o.HubID || '', HubName:o.HubName || '', LocationMapURL:o.LocationMapURL || '',
    Items:o.Items || '', ProductTotal:o.ProductTotal || 0, DeliveryCharge:o.DeliveryCharge || 0, GrandTotal:o.GrandTotal || 0,
    PaymentMode:o.PaymentMode || '', PaymentStatus:o.PaymentStatus || '', OrderStatus:o.OrderStatus || '', DeliveryOption:o.DeliveryOption || '', DeliverySlot:o.DeliverySlot || '', Notes:o.Notes || ''
  };
}

function orderItemsForOrder_(orderId){
  return rowsAsObjects_('OrderItems').filter(function(it){ return String(it.OrderID || '') === String(orderId || ''); });
}

function orderHistoryForOrder_(orderId){
  return rowsAsObjects_('OrderStatusHistory').filter(function(h){ return String(h.OrderID || '') === String(orderId || ''); });
}

function notificationLogsForOrder_(orderId){
  return rowsAsObjects_('WhatsAppNotifications').filter(function(n){ return String(n.OrderID || '') === String(orderId || ''); });
}

function trackOrder_(params){
  params = params || {};
  const orderId = String(params.OrderID || params.orderId || '').trim();
  const phone = cleanPhone_(params.Phone || params.phone || '');
  if(!orderId) return {ok:false,message:'Please enter order number.'};
  const order = findRowObject_('Orders','OrderID',orderId);
  if(!order) return {ok:false,message:'Order not found. Please check the order number.'};
  if(phone && cleanPhone_(order.Phone) !== phone && cleanPhone_(order.WhatsApp) !== phone){
    return {ok:false,message:'Phone number does not match this order.'};
  }
  const receipt = rowsAsObjects_('Receipts').find(function(r){ return String(r.OrderID || '') === String(orderId) && String(r.ReceiptType || '').indexOf('Customer') !== -1; }) || {};
  const out = publicOrder_(order);
  out.ReceiptID = receipt.ReceiptID || '';
  return {ok:true,order:out,items:orderItemsForOrder_(orderId),history:orderHistoryForOrder_(orderId),notifications:notificationLogsForOrder_(orderId),receipt:receipt};
}

function whatsappLink_(phone, message){
  const p = cleanPhone_(phone);
  if(!p) return '';
  const finalPhone = p.length === 10 ? '91'+p : p;
  return 'https://wa.me/'+finalPhone+'?text='+encodeURIComponent(message || '');
}

function buildOrderNotificationMessage_(order, type, remarks){
  order = order || {};
  const website = setting_('FreshlyMartWebsiteURL') || '';
  const track = website ? website.replace(/\/$/,'') + '/track-order.html?orderId=' + encodeURIComponent(order.OrderID || '') + '&phone=' + encodeURIComponent(cleanPhone_(order.Phone || '')) : '';
  return [
    'Freshly Mart Order Update', '',
    'Order ID: ' + (order.OrderID || ''),
    'Status: ' + (order.OrderStatus || 'New'),
    'Payment: ' + (order.PaymentStatus || ''),
    'Total: ₹' + toNumber_(order.GrandTotal).toFixed(2),
    'Hub: ' + (order.HubName || ''),
    order.DeliverySlot ? 'Slot: ' + order.DeliverySlot : '',
    remarks ? 'Remarks: ' + remarks : '',
    track ? 'Track Order: ' + track : '', '',
    'Thank you for shopping with Freshly Mart.'
  ].filter(Boolean).join('\n');
}

function queueWhatsAppNotification_(orderId, type, remarks, source){
  const order = findRowObject_('Orders','OrderID',orderId);
  if(!order) return {ok:false,message:'Order not found for WhatsApp notification.'};
  const message = buildOrderNotificationMessage_(order, type || 'Order Update', remarks || '');
  const link = whatsappLink_(order.WhatsApp || order.Phone, message);
  const mode = setting_('WhatsAppNotificationMode') || 'Link';
  const id = 'WANOT' + idStamp_();
  let status = 'Link Generated';
  let sentAt = '';
  if(String(mode).toLowerCase() === 'cloudapi'){
    const sent = trySendWhatsAppCloud_(order.WhatsApp || order.Phone, message);
    status = sent.ok ? 'Sent' : 'Cloud API Failed - Link Generated';
    sentAt = sent.ok ? new Date() : '';
  }
  appendObject_('WhatsAppNotifications', {NotificationID:id, DateTime:new Date(), OrderID:orderId, Phone:order.WhatsApp || order.Phone || '', CustomerName:order.CustomerName || '', NotificationType:type || 'Order Update', Message:message, WhatsAppLink:link, SendMode:mode, SendStatus:status, SentAt:sentAt, Remarks:remarks || source || ''});
  return {ok:true,NotificationID:id,WhatsAppLink:link,SendStatus:status,message:'WhatsApp notification '+status+'.'};
}

function trySendWhatsAppCloud_(phone, message){
  try{
    const token = setting_('WhatsAppCloudToken');
    const phoneNumberId = setting_('WhatsAppCloudPhoneNumberID');
    if(!token || !phoneNumberId) return {ok:false,message:'WhatsApp Cloud API settings missing'};
    const finalPhone = cleanPhone_(phone).length === 10 ? '91'+cleanPhone_(phone) : cleanPhone_(phone);
    const url = 'https://graph.facebook.com/v20.0/' + phoneNumberId + '/messages';
    const payload = {messaging_product:'whatsapp', to:finalPhone, type:'text', text:{preview_url:false, body:message}};
    const res = UrlFetchApp.fetch(url, {method:'post', contentType:'application/json', headers:{Authorization:'Bearer '+token}, payload:JSON.stringify(payload), muteHttpExceptions:true});
    const code = res.getResponseCode();
    return {ok:code>=200 && code<300, code:code, body:res.getContentText()};
  }catch(err){ return {ok:false,message:String(err)}; }
}

function updateOrderStatus_(d){
  d = d || {};
  validateRequired_(d, ['OrderID','OrderStatus'], 'Order status update');
  const order = findRowObject_('Orders','OrderID',d.OrderID);
  if(!order) throw new Error('Order not found: '+d.OrderID);
  const oldStatus = order.OrderStatus || '';
  const updates = {OrderStatus:d.OrderStatus};
  if(d.PaymentStatus) updates.PaymentStatus = d.PaymentStatus;
  if(d.DeliverySlot) updates.DeliverySlot = d.DeliverySlot;
  if(d.HubName) updates.HubName = d.HubName;
  updateRowById_('Orders','OrderID',d.OrderID,updates);
  appendObject_('OrderStatusHistory', {HistoryID:'HIST'+idStamp_(), DateTime:new Date(), OrderID:d.OrderID, OldStatus:oldStatus, NewStatus:d.OrderStatus, PaymentStatus:d.PaymentStatus || order.PaymentStatus || '', UpdatedBy:d.UpdatedBy || 'Admin', NotifyCustomer:d.NotifyCustomer || 'Yes', Remarks:d.Remarks || ''});
  let wa = null;
  if(String(d.NotifyCustomer || 'Yes').toLowerCase() !== 'no') wa = queueWhatsAppNotification_(d.OrderID, 'Order Status Update', d.Remarks || '', 'updateOrderStatus');
  logActivity_('updateOrderStatus','Admin','Order '+d.OrderID+' '+oldStatus+' -> '+d.OrderStatus);
  return {ok:true,message:'Order status updated.', WhatsAppLink:wa && wa.WhatsAppLink, notification:wa};
}

function isCancellationBlockedStatus_(status){
  const st = String(status || '').toLowerCase();
  return ['delivered','cancelled','refunded','out for delivery','cancellation approved','cancellation rejected'].indexOf(st) !== -1;
}

function orderPaidToFreshlyMart_(order){
  const ps = String((order && order.PaymentStatus) || '').toLowerCase();
  return ps === 'paid to freshly mart' || ps.indexOf('paid') !== -1;
}

function latestCancellationForOrder_(orderId){
  const rows = rowsAsObjectsWithRow_('Cancellations').filter(function(r){ return String(r.OrderID || '') === String(orderId || ''); });
  return rows.length ? rows[rows.length - 1] : null;
}

function buildCancellationMessage_(order, cancellation, type, remarks){
  order = order || {};
  cancellation = cancellation || {};
  const website = setting_('FreshlyMartWebsiteURL') || '';
  const track = website ? website.replace(/\/$/,'') + '/track-order.html?orderId=' + encodeURIComponent(order.OrderID || '') + '&phone=' + encodeURIComponent(cleanPhone_(order.Phone || '')) : '';
  return [
    'Freshly Mart Cancellation Update', '',
    'Order ID: ' + (order.OrderID || ''),
    'Cancellation Status: ' + (cancellation.CancellationStatus || type || ''),
    'Order Status: ' + (order.OrderStatus || ''),
    'Payment: ' + (order.PaymentStatus || ''),
    cancellation.RefundRequired === 'Yes' ? 'Refund Amount: ₹' + toNumber_(cancellation.RefundAmount).toFixed(2) : '',
    remarks ? 'Remarks: ' + remarks : '',
    track ? 'Track Order: ' + track : '', '',
    'Thank you for shopping with Freshly Mart.'
  ].filter(Boolean).join('\n');
...
function queueCancellationWhatsApp_(orderId, cancellation, type, remarks){
  const order = findRowObject_('Orders','OrderID',orderId);
  if(!order) return {ok:false,message:'Order not found for cancellation WhatsApp.'};
  const message = buildCancellationMessage_(order, cancellation || {}, type || 'Cancellation Update', remarks || '');
  const link = whatsappLink_(order.WhatsApp || order.Phone, message);
  const id = 'WANOT' + idStamp_();
  appendObject_('WhatsAppNotifications', {NotificationID:id, DateTime:new Date(), OrderID:orderId, Phone:order.WhatsApp || order.Phone || '', CustomerName:order.CustomerName || '', NotificationType:type || 'Cancellation Update', Message:message, WhatsAppLink:link, SendMode:setting_('WhatsAppNotificationMode') || 'Link', SendStatus:'Link Generated', SentAt:'', Remarks:remarks || ''});
  return {ok:true,NotificationID:id,WhatsAppLink:link,SendStatus:'Link Generated',message:'Cancellation WhatsApp link generated.'};
}

function requestCancellation_(d){
  d = d || {};
  validateRequired_(d, ['OrderID','Phone','Reason'], 'Cancellation request');
  const order = findRowObject_('Orders','OrderID',d.OrderID);
  if(!order) throw new Error('Order not found: '+d.OrderID);
  const reqPhone = cleanPhone_(d.Phone);
  if(reqPhone && cleanPhone_(order.Phone) !== reqPhone && cleanPhone_(order.WhatsApp) !== reqPhone) throw new Error('Phone number does not match this order.');
  if(isCancellationBlockedStatus_(order.OrderStatus)) throw new Error('Cancellation is not available for this order stage. Please use return/refund support if applicable.');
  const existing = latestCancellationForOrder_(d.OrderID);
  if(existing && ['cancellation requested','refund pending'].indexOf(String(existing.CancellationStatus || '').toLowerCase()) !== -1){
    return {ok:true,message:'Cancellation request is already submitted for this order.', CancellationID:existing.CancellationID};
  }
  const paid = orderPaidToFreshlyMart_(order);
  const id = 'CAN' + idStamp_();
  const cancellation = {
    CancellationID:id, DateTime:new Date(), OrderID:order.OrderID, CustomerName:order.CustomerName || d.CustomerName || '', Phone:order.Phone || d.Phone || '',
    Reason:d.Reason || '', PaymentStatus:order.PaymentStatus || '', OrderStatusAtRequest:order.OrderStatus || '', RefundRequired:paid ? 'Yes' : 'No',
    RefundAmount: paid ? toNumber_(order.GrandTotal) : 0, RefundUPIID:d.RefundUPIID || '', RequestDate:new Date(), CancellationStatus:'Cancellation Requested',
    ApprovedBy:'', ApprovedDate:'', AdminRemarks:d.Remarks || '', WhatsAppLink:'', SourcePage:d.SourcePage || ''
  };
  appendObject_('Cancellations', cancellation);
  updateRowById_('Orders','OrderID',order.OrderID,{OrderStatus:'Cancellation Requested'});
  appendObject_('OrderStatusHistory', {HistoryID:'HIST'+idStamp_(), DateTime:new Date(), OrderID:order.OrderID, OldStatus:order.OrderStatus || '', NewStatus:'Cancellation Requested', PaymentStatus:order.PaymentStatus || '', UpdatedBy:'Customer', NotifyCustomer:'Yes', Remarks:'Cancellation requested: '+(d.Reason || '')});
  const wa = queueCancellationWhatsApp_(order.OrderID, cancellation, 'Cancellation Requested', 'We have received your cancellation request. Freshly Mart admin will review it shortly.');
  updateRowById_('Cancellations','CancellationID',id,{WhatsAppLink:wa.WhatsAppLink || ''});
  logActivity_('requestCancellation','Customer','Order '+order.OrderID+' cancellation requested');
  return {ok:true,message:'Cancellation request submitted. Freshly Mart will review and update you.', CancellationID:id, WhatsAppLink:wa.WhatsAppLink};
}

function approveCancellation_(d){
  d = d || {};
  const rec = d.CancellationID ? findRowObject_('Cancellations','CancellationID',d.CancellationID) : latestCancellationForOrder_(d.OrderID);
  if(!rec) throw new Error('Cancellation request not found. Enter Cancellation ID or Order ID.');
  const order = findRowObject_('Orders','OrderID',rec.OrderID);
  if(!order) throw new Error('Order not found: '+rec.OrderID);
  const paid = orderPaidToFreshlyMart_(order) || String(rec.RefundRequired || '').toLowerCase() === 'yes';
  const paymentStatus = paid ? 'Refund Pending' : 'Cancelled - No Payment';
  updateRowById_('Orders','OrderID',rec.OrderID,{OrderStatus:'Cancelled', PaymentStatus:paymentStatus});
  updateRowById_('Cancellations','CancellationID',rec.CancellationID,{CancellationStatus: paid ? 'Refund Pending' : 'Cancelled', ApprovedBy:d.ApprovedBy || 'Admin', ApprovedDate:new Date(), AdminRemarks:d.AdminRemarks || ''});
  appendObject_('OrderStatusHistory', {HistoryID:'HIST'+idStamp_(), DateTime:new Date(), OrderID:rec.OrderID, OldStatus:order.OrderStatus || '', NewStatus:'Cancelled', PaymentStatus:paymentStatus, UpdatedBy:d.ApprovedBy || 'Admin', NotifyCustomer:d.NotifyCustomer || 'Yes', Remarks:d.AdminRemarks || 'Cancellation approved'});
  let refund = null;
  if(paid){
    refund = {RefundID:'REF'+idStamp_(), DateTime:new Date(), ReturnID:'', OrderID:rec.OrderID, CustomerName:order.CustomerName || rec.CustomerName || '', RefundAmount:toNumber_(rec.RefundAmount || order.GrandTotal), RefundMode:'UPI', RefundStatus:'Pending', RefundDate:'', UPIReference:'', RefundReason:'Cancellation', Remarks:'Cancellation approved. Refund UPI: '+(rec.RefundUPIID || '')};
    appendObject_('Refunds', refund);
  }
  let wa = null;
  if(String(d.NotifyCustomer || 'Yes').toLowerCase() !== 'no') wa = queueCancellationWhatsApp_(rec.OrderID, Object.assign({}, rec, {CancellationStatus: paid ? 'Refund Pending' : 'Cancelled'}), 'Cancellation Approved', d.AdminRemarks || 'Your order cancellation has been approved.');
  logActivity_('approveCancellation','Admin','Order '+rec.OrderID+' cancellation approved');
  return {ok:true,message: paid ? 'Cancellation approved. Refund record created.' : 'Cancellation approved. No refund required.', refund:refund, WhatsAppLink:wa && wa.WhatsAppLink};
}

function rejectCancellation_(d){
  d = d || {};
  const rec = d.CancellationID ? findRowObject_('Cancellations','CancellationID',d.CancellationID) : latestCancellationForOrder_(d.OrderID);
  if(!rec) throw new Error('Cancellation request not found. Enter Cancellation ID or Order ID.');
  const order = findRowObject_('Orders','OrderID',rec.OrderID);
  if(!order) throw new Error('Order not found: '+rec.OrderID);
  const restoreStatus = rec.OrderStatusAtRequest || 'Confirmed';
  updateRowById_('Orders','OrderID',rec.OrderID,{OrderStatus:restoreStatus});
  updateRowById_('Cancellations','CancellationID',rec.CancellationID,{CancellationStatus:'Cancellation Rejected', ApprovedBy:d.ApprovedBy || 'Admin', ApprovedDate:new Date(), AdminRemarks:d.AdminRemarks || ''});
  appendObject_('OrderStatusHistory', {HistoryID:'HIST'+idStamp_(), DateTime:new Date(), OrderID:rec.OrderID, OldStatus:order.OrderStatus || '', NewStatus:restoreStatus, PaymentStatus:order.PaymentStatus || '', UpdatedBy:d.ApprovedBy || 'Admin', NotifyCustomer:d.NotifyCustomer || 'Yes', Remarks:'Cancellation rejected: '+(d.AdminRemarks || '')});
  let wa = null;
  if(String(d.NotifyCustomer || 'Yes').toLowerCase() !== 'no') wa = queueCancellationWhatsApp_(rec.OrderID, Object.assign({}, rec, {CancellationStatus:'Cancellation Rejected'}), 'Cancellation Rejected', d.AdminRemarks || 'Cancellation could not be approved at this stage.');
  logActivity_('rejectCancellation','Admin','Order '+rec.OrderID+' cancellation rejected');
  return {ok:true,message:'Cancellation rejected and order status restored.', WhatsAppLink:wa && wa.WhatsAppLink};
}

function reviewCancellationPrompt(){
  const ui = SpreadsheetApp.getUi();
  const orderRes = ui.prompt('Review Cancellation', 'Enter Order ID:', ui.ButtonSet.OK_CANCEL);
  if(orderRes.getSelectedButton() !== ui.Button.OK) return;
  const choice = ui.alert('Cancellation Review', 'Approve cancellation for '+orderRes.getResponseText()+'?', ui.ButtonSet.YES_NO_CANCEL);
  if(choice === ui.Button.YES){
    const remarks = ui.prompt('Approval Remarks', 'Enter admin remarks:', ui.ButtonSet.OK_CANCEL);
    const result = approveCancellation_({OrderID:orderRes.getResponseText(), ApprovedBy:'Admin', AdminRemarks:remarks.getSelectedButton()===ui.Button.OK ? remarks.getResponseText() : '', NotifyCustomer:'Yes'});
    ui.alert(result.message);
  }else if(choice === ui.Button.NO){
    const remarks = ui.prompt('Rejection Reason', 'Enter rejection reason:', ui.ButtonSet.OK_CANCEL);
    const result = rejectCancellation_({OrderID:orderRes.getResponseText(), ApprovedBy:'Admin', AdminRemarks:remarks.getSelectedButton()===ui.Button.OK ? remarks.getResponseText() : 'Rejected by admin', NotifyCustomer:'Yes'});
    ui.alert(result.message);
  }
}
...
function saveOrder_(d){
  ...
  let linkedCustomer = d.CustomerID ? findRowObject_('Customers','CustomerID',d.CustomerID) : null;
  if(!linkedCustomer && d.Phone) linkedCustomer = findCustomerByPhone_(d.Phone);
  if(linkedCustomer){
    d.CustomerID = linkedCustomer.CustomerID || d.CustomerID || '';
  }
  ...
  if(d.CurrentLatitude && d.CurrentLongitude && !d.LocationMapURL){
    d.LocationMapURL = 'https://www.google.com/maps?q=' + d.CurrentLatitude + ',' + d.CurrentLongitude;
  }
  if(d.CustomerID){
    try{ updateRowById_('Customers','CustomerID',d.CustomerID,{PreferredHubID:d.HubID, PreferredHubName:d.HubName, Pincode:d.Pincode, Area:d.Area, CurrentLatitude:d.CurrentLatitude || '', CurrentLongitude:d.CurrentLongitude || '', LocationMapURL:d.LocationMapURL || ''}); }catch(err){}
  }
  ...
  appendObject_('OrderStatusHistory', {HistoryID:'HIST'+idStamp_(), DateTime:new Date(), OrderID:id, OldStatus:'', NewStatus:'New', PaymentStatus:paymentStatus, UpdatedBy:'System', NotifyCustomer:'Yes', Remarks:'Order placed'});
  let items = [];
  ...
  const notify = queueWhatsAppNotification_(id, 'Order Placed', 'Order received by Freshly Mart.', 'saveOrder');
  const adminMessage = buildOrderNotificationMessage_(findRowObject_('Orders','OrderID',id), 'Admin New Order', 'New order received. Please review and confirm.');
  const adminWhatsAppLink = whatsappLink_(setting_('AdminWhatsApp') || '918921696649', adminMessage);
  logActivity_('saveOrder','Public', 'Order '+id+' assigned to '+d.HubID+' for pincode '+d.Pincode);
  return {ok:true,message:'Order saved successfully and assigned to '+(d.HubName || 'Freshly Hub')+'.', OrderID:id, HubID:d.HubID, HubName:d.HubName, DeliveryCharge:d.DeliveryCharge, GrandTotal:d.GrandTotal, DeliverySlot:d.DeliverySlot, OrderStatus:'New', PaymentStatus:paymentStatus, customerWhatsAppLink:notify.WhatsAppLink || '', adminWhatsAppLink:adminWhatsAppLink};
}
...
function adminSummary_(){
  ...
  const cancellations = rowsAsObjects_('Cancellations').filter(function(r){
    const st = String(r.CancellationStatus || '').toLowerCase();
    return st === 'cancellation requested' || st === 'refund pending' || !st;
  }).slice(-20).reverse();
  const reviews = rowsAsObjects_('Reviews').filter(function(r){ return String(r.Status).toLowerCase()==='pending'; }).slice(0,20);
  return {ok:true, counts:{PendingSellers:sellers.length, PendingProducts:products.length, Returns:returns.length, Cancellations:cancellations.length, Reviews:reviews.length}, pendingSellers:sellers, pendingProducts:products, returns:returns, cancellations:cancellations, reviews:reviews};
}
...
function updateOrderStatusPrompt(){
  const ui = SpreadsheetApp.getUi();
  const orderRes = ui.prompt('Update Order Status', 'Enter Order ID', ui.ButtonSet.OK_CANCEL);
  if(orderRes.getSelectedButton() !== ui.Button.OK) return;
  const statusRes = ui.prompt('Update Order Status', 'Enter new status (New / Confirmed / Procurement Pending / Sent to Hub / Out for Delivery / Delivered / Cancelled)', ui.ButtonSet.OK_CANCEL);
  if(statusRes.getSelectedButton() !== ui.Button.OK) return;
  const remarksRes = ui.prompt('WhatsApp Notification', 'Remarks to include in customer WhatsApp notification', ui.ButtonSet.OK_CANCEL);
  const result = updateOrderStatus_({OrderID:orderRes.getResponseText(), OrderStatus:statusRes.getResponseText(), Remarks:remarksRes.getResponseText(), NotifyCustomer:'Yes', UpdatedBy:'Admin'});
  safeUiAlert_(result.message + (result.WhatsAppLink ? '\n\nWhatsApp link generated in WhatsAppNotifications sheet.' : ''));
}
...
function applyBackendValidations(showAlert){
  ...
  applyValidationToColumn_('Customers','Status', ['Active','Inactive','Blocked']);
...
function setting_(name){ return getSetting_(name); }
