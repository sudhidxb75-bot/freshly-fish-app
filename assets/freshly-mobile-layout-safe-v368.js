(function(){
  ...
  function cssVar(el, name, fallback){
    ...
    const inline = el.style && el.style.getPropertyValue(name);
    const computed = getComputedStyle(el).getPropertyValue(name);
    return String(inline || computed || fallback || '').trim();
  }

  function mobileBannerHeight(slider){
    const slide = slider && (slider.querySelector('.promo-slide:not(.banner-image-error)') || slider.querySelector('.promo-slide'));
    const raw = cssVar(slide, '--banner-mobile-height', cssVar(slider, '--banner-mobile-height', '270px'));
    const n = parseInt(String(raw || '').replace(/[^0-9]/g,''), 10);
    return (Number.isFinite(n) && n >= 120 && n <= 520 ? n : 270) + 'px';
  }

  function mobileObjectFit(slide){
    const v = cssVar(slide, '--banner-mobile-object-fit', 'cover').toLowerCase();
    return ['cover','contain','fill'].indexOf(v) >= 0 ? v : 'cover';
  }

  function mobileObjectPosition(slide){
    return cssVar(slide, '--banner-mobile-object-position', 'center top') || 'center top';
  }
  ...
  function applyLayout(){
    ...
    requestAnimationFrame(function(){
      ...
      if(slider){
        const bannerHeight = mobileBannerHeight(slider);
        ...
        if(!visibleSlides.length){
          ...
          if(waitingForBackendBanners){
            ...
            slider.style.height = bannerHeight;
            slider.style.maxHeight = bannerHeight;
        ...
        if(track){
          ...
          track.style.height = visibleSlides.length ? bannerHeight : '0';
        ...
        slides.forEach(function(slide){
          ...
          slide.style.height = show ? bannerHeight : '0';
          slide.style.minHeight = show ? bannerHeight : '0';
          slide.style.maxHeight = show ? bannerHeight : '0';
          ...
          slide.querySelectorAll('img').forEach(function(img){
            ...
            img.style.objectFit = mobileObjectFit(slide);
            img.style.objectPosition = mobileObjectPosition(slide);
