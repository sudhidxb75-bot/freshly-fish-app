(() => {
  ...
  const CATEGORY_LABELS = [
    ['Fish & Seafood','🐟 Fish & Seafood'], ['Chicken','🍗 Chicken'], ['Mutton','🥩 Mutton'], ['Eggs','🥚 Eggs'],
    ['Fruits & Vegetables','🥦 Fruits & Veg'], ['Food','🍱 Food'], ['Groceries','🍚 Groceries'], ['Daily Essentials','🛒 Essentials'],
    ['Ready to Cook','🍳 Ready to Cook'], ['Combo Packs','🏷️ Combo Packs'], ['Freshly Mart','🛍️ Freshly Mart']
  ];
  ...
  function openCategory(category, fromOverlay=false){
    ...
    setTimeout(() => {
      ...
      if(fromOverlay) setTimeout(closeMenu, 90);
    }, fromOverlay ? 140 : 0);
  }
  ...
  function buildMobileHome(){
    ...
    section.innerHTML = `<div class="freshly-greeting-card"><div><p class="eyebrow">${greeting}</p><h1>Freshness delivered near you</h1><p>Shop fresh fish, meat, groceries and daily essentials from Freshly hubs.</p></div><button type="button" data-go-shop>Shop now</button></div><div class="freshly-mobile-category-grid">${CATEGORY_LABELS.slice(0,8).map(([cat,label])=>`<button type="button" data-mobile-category="${cat}">${label}</button>`).join('')}</div>`;
  ...
  function buildMoreMenu(){
    ...
    modal.innerHTML = `<div class="freshly-more-sheet"><div class="sheet-head"><b>Freshly Menu</b><button type="button" data-close-more>×</button></div><div class="freshly-more-category-grid">${CATEGORY_LABELS.map(([cat,label])=>`<button type="button" data-mobile-category="${cat}">${label}</button>`).join('')}</div><div class="sheet-links"><a href="track-order.html">Track Order</a><a href="customer-portal.html">Customer Login</a><a href="partner-dashboard.html">Partner Portal</a><a href="admin.html">Admin</a><a href="contact.html">Contact Freshly</a></div></div>`;
  ...
  function buildBottomNav(){
    ...
    nav.innerHTML = `<button type="button" data-bottom-nav="home"><span>🏠</span><small>Home</small></button><button type="button" data-bottom-nav="shop"><span>🛒</span><small>Shop</small></button><button type="button" data-bottom-nav="cart" data-open-cart><span>🧺</span><small>Cart</small></button><a href="track-order.html" data-bottom-nav="orders"><span>📦</span><small>Orders</small></a><button type="button" data-bottom-nav="menu"><span>☰</span><small>Menu</small></button>`;
    document.body.appendChild(nav);
  }

  function bindMobileEvents(){
    $$('.mobile-toggle,[data-menu-toggle],.menu-toggle').forEach(btn => {
      if(btn.dataset.coreBoundV381 === 'yes') return;
      btn.dataset.coreBoundV381 = 'yes';
    ...
    document.addEventListener('click', e => {
      const dropdownBtn = e.target.closest('.menu .dropdown > button, .nav .menu .dropdown > button');
