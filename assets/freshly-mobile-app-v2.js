...
const FRESHLY_MART_URL='freshlymart/index.html';
function openFreshlyMartLink(){window.open(FRESHLY_MART_URL,'_blank','noopener')}
...
function openCategory(category){
  ...
  if(term.toLowerCase()==='freshly mart'){openFreshlyMartLink();return;}
...
function init(){
  ...
  document.addEventListener('click',e=>{const mart=e.target.closest('[data-freshlymart-link]');if(mart){e.preventDefault();e.stopPropagation();openFreshlyMartLink();return;}const cat=e.target.closest('[data-mobile-category]');if(cat){e.preventDefault();openCategory(cat.dataset.mobileCategory)}});
  document.addEventListener('keydown',e=>{const mart=e.target.closest&&e.target.closest('[data-freshlymart-link]');if(mart&&(e.key==='Enter'||e.key===' ')){e.preventDefault();openFreshlyMartLink();}});
...
(function(){
  function openCategoryFromTop(category){
    ...
    if(!term) return;
    if(term.toLowerCase()==='freshly mart'){ window.open('freshlymart/index.html','_blank','noopener'); return; }
    if(window.freshlyOpenCategory){ window.freshlyOpenCategory(term); return; }
