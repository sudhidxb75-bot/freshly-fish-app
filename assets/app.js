...
function parseCutoffTime(cutoffText){
  ...
  if(/^0?\.\d+$/.test(text)){
    ...
    if(!isNaN(decimal)){
      ...
      cutoff.setHours(Math.floor(totalMinutes / 60) % 24, totalMinutes % 60, 0, 0);
      return cutoff;
    }
  }
...
function formatCutoffTime(cutoffText){
  const cutoff = parseCutoffTime(cutoffText);
  if(!cutoff) return String(cutoffText || '').trim();

  let hour = cutoff.getHours();
  const minute = cutoff.getMinutes();
