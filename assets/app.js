(function(){
  ...
  function orderSuccessHtml_(res, order){
    const orderId = esc(res.orderId || res.OrderID || res.OrderFreshlyID || '');
    const hub = selectedHubRow_();
    const hubName = esc(hub?.HubName || order.HubID || 'Freshly Hub');
    const total = currency + num(res.total || res.Total || cartTotal());
    ...
    return `<div class="success-card card">
      ...
      <p>${orderId ? `Order ID: <b>${orderId}</b>` : 'Freshly has received your order.'}</p>
      <p><b>Hub:</b> ${hubName}<br><b>Total:</b> ${total}<br><b>Payment:</b> ${esc(order.PaymentMode || '')}</p>
      <p class="muted">Freshly will share payment and delivery updates. Please pay only through Freshly UPI / Freshly QR.</p>
      <a class="btn btn-primary" href="${trackLink}">Track Order</a>
    </div>`;
  }
  ...
  async function placeOrder(fd){
    ...
    try{
      ...
      const res = cfg.BACKEND_URL
        ...
        : {ok:true, orderId:'FLY-ORD-DEMO-' + Date.now(), message:'Demo order received.'};
