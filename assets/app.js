(function(){
  ...
  const demo = {
    ...
    productOptions:[
      {OptionID:'OPT-DEMO-001',ProductID:'FLY-PRD-000001',OptionType:'Cleaning',OptionName:'No Cleaning',AdditionalCharge:0,Status:'Active'},
      {OptionID:'OPT-DEMO-002',ProductID:'FLY-PRD-000001',OptionType:'Cleaning',OptionName:'Cleaned',AdditionalCharge:20,Status:'Active'},
      {OptionID:'OPT-DEMO-003',ProductID:'FLY-PRD-000001',OptionType:'Cutting',OptionName:'Whole Fish',AdditionalCharge:0,Status:'Active'},
      {OptionID:'OPT-DEMO-004',ProductID:'FLY-PRD-000001',OptionType:'Cutting',OptionName:'Curry Cut',AdditionalCharge:30,Status:'Active'},
      {OptionID:'OPT-DEMO-005',ProductID:'FLY-PRD-000002',OptionType:'Cleaning',OptionName:'No Cleaning',AdditionalCharge:0,Status:'Active'},
      {OptionID:'OPT-DEMO-006',ProductID:'FLY-PRD-000002',OptionType:'Cleaning',OptionName:'Cleaned',AdditionalCharge:40,Status:'Active'},
      {OptionID:'OPT-DEMO-007',ProductID:'FLY-PRD-000002',OptionType:'Cutting',OptionName:'Steak Cut',AdditionalCharge:0,Status:'Active'},
      {OptionID:'OPT-DEMO-008',ProductID:'FLY-PRD-000002',OptionType:'Cutting',OptionName:'Curry Cut',AdditionalCharge:50,Status:'Active'},
      {OptionID:'OPT-DEMO-009',ProductID:'FLY-PRD-000002',OptionType:'Marination',OptionName:'No Marination',AdditionalCharge:0,Status:'Active'},
      {OptionID:'OPT-DEMO-010',ProductID:'FLY-PRD-000002',OptionType:'Marination',OptionName:'Basic Marination',AdditionalCharge:60,Status:'Active'},
      {OptionID:'OPT-DEMO-011',ProductID:'FLY-PRD-000002',OptionType:'Marination',OptionName:'Kerala Fish Fry Masala Marination',AdditionalCharge:100,Status:'Active'},
      {OptionID:'OPT-DEMO-012',ProductID:'FLY-PRD-000002',OptionType:'Marination',OptionName:'Kerala Fish Curry Masala Marination',AdditionalCharge:100,Status:'Active'}
    ],
    ...
    subcategories:[
      ...
      {SubCategoryID:'SUB-SNACKS',CategoryID:'CAT-FOOD',Name:'Snacks',SortOrder:2,Status:'Active'},
      {SubCategoryID:'SUB-RICE-GRAINS',CategoryID:'CAT-GROCERY',Name:'Rice & Grains',SortOrder:1,Status:'Active'},
      {SubCategoryID:'SUB-SPICES',CategoryID:'CAT-GROCERY',Name:'Spices & Masala',SortOrder:2,Status:'Active'},
      {SubCategoryID:'SUB-HOME-PRODUCTS',CategoryID:'CAT-FRESHLYMART',Name:'Home Products',SortOrder:1,Status:'Active'},
      {SubCategoryID:'SUB-STATIONERY',CategoryID:'CAT-FRESHLYMART',Name:'Stationery',SortOrder:2,Status:'Active'},
      {SubCategoryID:'SUB-ELECTRONICS-ACCESSORIES',CategoryID:'CAT-FRESHLYMART',Name:'Electronics Accessories',SortOrder:3,Status:'Active'}
    ],
    products:[
  ...
  function init(){ setYear(); applyConfiguredContacts(); bindNav(); bindLocationDropdown(); bindPromoSlider(); bindBannerResponsiveRefresh(); bindGenericForms(); bindPortalForms(); bindCustomerAuthTabs(); bindTrackOrder(); bindCustomerOrders(); bindCheckout(); bindCheckoutLocation(); bindAdmin(); if(document.querySelector('#shop') || document.querySelector('#productGrid')) loadPublicData(); updateCustomerUI(); updateCartUI(); if(document.querySelector('#customerOrdersBox')) loadCustomerOrders(); }
  ...
  function normalizeBanner_(b, source, idx){
    ...
    return Object.assign({}, b, {
      DesktopImageURL: desktopImage || mobileImage,
  ...
  function bindBannerResponsiveRefresh(){
    if(window.freshlyBannerResizeBound) return;
    window.freshlyBannerResizeBound = true;
    let lastMode = null;
    let timer = null;
    const check = () => {
      const mode = isMobileBannerView_() ? 'mobile' : 'desktop';
      if(lastMode === null){ lastMode = mode; return; }
      if(mode !== lastMode){ lastMode = mode; renderPromoSlider(); }
    };
    check();
  ...
  function renderPromoSlider(){
    ...
    let banners = normalizeBanners_(backendRows.length ? backendRows : demoRows, backendRows.length ? 'backend' : 'local')
      .filter(b => String(b.DesktopImageURL || b.MobileImageURL || b.ImageURL || b.Title || '').trim())
    ...
    if(isMobileBannerView_()){
      const imageBanners = banners.filter(b => String(b.MobileImageURL || b.DesktopImageURL || b.ImageURL || '').trim());
    ...
    if(slider){
      ...
      const parentDesktopHeight = parseInt(String(firstBanner.DesktopHeight || firstBanner.BannerHeight || firstBanner.Height || '340').replace(/[^0-9]/g,''), 10) || 340;
      const parentMobileHeight = parseInt(String(firstBanner.MobileHeight || firstBanner.MobileBannerHeight || '270').replace(/[^0-9]/g,''), 10) || 270;
      slider.style.setProperty('--banner-desktop-height', parentDesktopHeight + 'px');
      slider.style.setProperty('--banner-mobile-height', parentMobileHeight + 'px');
      slider.classList.remove('hidden');
      slider.style.overflow = 'hidden';
      slider.style.width = '100%';
      slider.dataset.bannerCount = String(banners.length);
      slider.dataset.bannerMode = isMobileBannerView_() ? 'mobile' : 'desktop';
    }
  ...
  function promoSlide(b, idx){
    const desktopImage = normalizeBannerImageUrl_(b.DesktopImageURL || bannerImageUrl_(b));
    const mobileImage = normalizeBannerImageUrl_(b.MobileImageURL || desktopImage);
    const image = isMobileBannerView_() ? (mobileImage || desktopImage) : (desktopImage || mobileImage);
    const imageErrorFallback = (isMobileBannerView_() && desktopImage && mobileImage && mobileImage !== desktopImage) ? desktopImage : (b._FallbackImage || fallbackBannerImage_(idx));
    ...
    if(image && (displayMode === 'imageonly' || hideTextOverlay || !wantsTextOverlay)){
      ...
      return `<article class="promo-slide backend-banner image-only" style="${cssVars}" data-banner-source="${esc(b._BannerSource || '')}">
        <a class="banner-image-link" href="${btnLink}" aria-label="${title}"${linkAttrs}>
          <img data-banner-img src="${esc(image)}" data-fallback-src="${esc(imageErrorFallback)}" data-desktop-src="${esc(desktopImage)}" data-mobile-src="${esc(mobileImage)}" alt="${title}"
  ...
  async function loadPublicData(){
    try{
      ...
      state.categories = p.categories || p.Categories || p.categoryRows || demo.categories || [];
      state.subcategories = p.subcategories || p.subCategories || p.SubCategories || p.subCategoryRows || demo.subcategories || [];
      state.products = p.products || p.Products || p.productRows || demo.products || [];
      state.productOptions = p.productOptions || p.ProductOptions || p.options || p.productOptionRows || demo.productOptions || [];
      state.districts = p.districts || p.Districts || demo.districts || [];
      state.hubs = p.hubs || p.Hubs || demo.hubs || [];
      state.areas = p.areas || p.Areas || demo.areas || [];
      state.slots = p.slots || p.HubDeliverySlots || p.hubDeliverySlots || demo.slots || [];
      state.districtPricing = p.districtPricing || p.DistrictPricing || p.pricing || demo.districtPricing || [];
  ...
  function productCategoryValues_(p){
    return [
      valueFrom_(p, ['CategoryID','Category ID','CategoryCode','Category Code']),
      valueFrom_(p, ['Category','CategoryName','Category Name','MainCategory','Main Category','ProductCategory','Product Category','Department']),
      p.CategoryID, p.Category, p.CategoryName, p.CategoryLabel, p.MainCategory, p.ProductCategory, p.Department
  ...
  function subcategoryId_(s){ return valueFrom_(s, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','SubCategoryCode','Sub Category Code','ID','Code']) || valueFrom_(s, ['Name','SubCategoryName','Sub Category Name','SubCategory','Sub Category','Title']); }
  function subcategoryName_(s){ return valueFrom_(s, ['Name','SubCategoryName','Sub Category Name','SubCategory','Sub Category','Title']) || valueFrom_(s, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','SubCategoryCode','Sub Category Code','ID','Code']); }
  function subcategoryCategory_(s){ return valueFrom_(s, ['CategoryID','Category ID','ParentCategoryID','Parent Category ID','ParentCategory','Parent Category','Category','CategoryName','Category Name','MainCategory','Main Category']); }
  function productSubcategoryId_(p){
    return valueFrom_(p, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','SubCategoryCode','Sub Category Code']) ||
      valueFrom_(p, ['SubCategory','Sub Category','SubCategoryName','Sub Category Name','ProductSubCategory','Product SubCategory','Product Sub Category','ChildCategory','Child Category','ChildSubCategory','Child SubCategory','Child Sub Category','ItemSubCategory','Item SubCategory','Item Sub Category']);
  }
  function productSubcategoryName_(p){
    return valueFrom_(p, ['SubCategory','Sub Category','SubCategoryName','Sub Category Name','ProductSubCategory','Product SubCategory','Product Sub Category','ChildCategory','Child Category','ChildSubCategory','Child SubCategory','Child Sub Category','ItemSubCategory','Item SubCategory','Item Sub Category']) ||
      valueFrom_(p, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','SubCategoryCode','Sub Category Code']);
  }
  function productSubcategoryValues_(p){
    return [
      productSubcategoryId_(p),
      productSubcategoryName_(p),
      p.SubCategoryID, p.SubCategory, p.SubCategoryName, p.ProductSubCategory,
      p.ChildCategory, p.ChildSubCategory, p.ItemSubCategory
    ].filter(v => String(v || '').trim());
  }
  function subcategoryRouteLabels_(selected){
    if(!selected || selected === 'all') return [];
    const raw = String(selected || '').trim();
    const vals = [raw];
    activeRows(state.subcategories).forEach(s => {
      const sid = subcategoryId_(s);
      const name = subcategoryName_(s);
      if(categoryKey_(sid) === categoryKey_(raw) || categoryKey_(name) === categoryKey_(raw)) vals.push(sid, name);
    });
    return [...new Set(vals.map(categoryKey_).filter(Boolean))];
  }
  ...
  function hasSpecificCategorySelected_(){
    return !!state.selectedCategory && categoryKey_(state.selectedCategory) !== 'all';
  }
  ...
  function getAvailableSubcategories_(){
    if(!hasSpecificCategorySelected_()) return [];
    const seen = {};
    const out = [];
    const selectedLabels = categoryRouteLabels_(state.selectedCategory);
    const matchingProducts = activeRows(state.products).filter(p => productMatchesCategory_(p, state.selectedCategory));
    const productSubKeys = new Set();

    matchingProducts.forEach(p => {
      productSubcategoryValues_(p).forEach(v => {
        const key = categoryKey_(v);
        if(key) productSubKeys.add(key);
      });
    });

    function add(id, name, categoryId, sortOrder){
      id = String(id || name || '').trim(); name = String(name || id || '').trim();
      if(!id || !name) return;
      const key = categoryKey_(id || name);
      if(seen[key]) return;
      seen[key] = true;
    ...
    activeRows(state.subcategories).forEach(s => {
      ...
      const sKeys = [sid, sname].map(categoryKey_).filter(Boolean);
      const directCategoryMatch = subcategoryBelongsToCategory_(s, state.selectedCategory);
      const productUsesThisSubcat = sKeys.some(k => productSubKeys.has(k));
      const looseCategoryTextMatch = selectedLabels.some(l => {
        const cat = categoryKey_(subcategoryCategory_(s));
        return cat && (cat === l || cat.includes(l) || l.includes(cat));
      });
      if(directCategoryMatch || productUsesThisSubcat || looseCategoryTextMatch){
        add(sid, sname, subcategoryCategory_(s), s.SortOrder);
      }
    });

    matchingProducts.forEach(p => {
      const id = productSubcategoryId_(p);
  ...
  function ensureSubcategoryContainer_(){
    let box = document.querySelector('#subcategoryTabs');
    if(box) return box;
    const cats = document.querySelector('#categoryTabs');
    if(!cats || !cats.parentNode) return null;
    box = document.createElement('div');
    box.id = 'subcategoryTabs';
    box.className = 'subcategory-tabs';
    cats.parentNode.insertBefore(box, cats.nextSibling);
    return box;
  }
  function renderSubCategories(){
    const box = ensureSubcategoryContainer_();
    if(!box) return;

    function hideSubcategoryRow_(){
    ...
    // Clean shop view: do not show every subcategory under the All category.
    // Subcategories appear only after the customer selects a specific main category.
    if(!hasSpecificCategorySelected_()){
      hideSubcategoryRow_();
      return;
    }

    const subs = getAvailableSubcategories_();
    if(!subs.length){
      hideSubcategoryRow_();
      return;
    }

    const currentLabels = subcategoryRouteLabels_(state.selectedSubCategory);
    if(state.selectedSubCategory !== 'all' && !subs.some(s => currentLabels.includes(categoryKey_(s.id)) || currentLabels.includes(categoryKey_(s.name)))){
      state.selectedSubCategory = 'all';
      localStorage.setItem('freshlySelectedSubCategory','all');
    }
    ...
    const selectedLabels = subcategoryRouteLabels_(state.selectedSubCategory);
    box.innerHTML = `<span class="subcat-label">Subcategory</span><button class="tab subtab ${state.selectedSubCategory==='all'?'active':''}" data-subcat="all">All</button>` + subs.map(s=>{
      const active = state.selectedSubCategory !== 'all' && (selectedLabels.includes(categoryKey_(s.id)) || selectedLabels.includes(categoryKey_(s.name)));
      return `<button class="tab subtab ${active?'active':''}" data-subcat="${esc(s.id)}">${esc(s.name)}</button>`;
    }).join('');
    box.querySelectorAll('[data-subcat]').forEach(b=>b.onclick=()=>{
      state.selectedSubCategory = b.dataset.subcat || 'all';
  ...
  window.freshlyDebugSubcategories = function(){
    return {
      selectedCategory: state.selectedCategory,
      selectedSubCategory: state.selectedSubCategory,
      categories: state.categories,
      subcategories: state.subcategories,
      availableSubcategories: getAvailableSubcategories_(),
      matchingProducts: activeRows(state.products).filter(p => productMatchesCategory_(p, state.selectedCategory)).map(p => ({Name:p.Name, CategoryID:valueFrom_(p,['CategoryID','Category ID','Category']), SubCategoryID:productSubcategoryId_(p), SubCategory:productSubcategoryName_(p)}))
    };
  };
  ...
  function renderCategories(){
    ...
    box.innerHTML = `<button class="tab ${state.selectedCategory==='all'?'active':''}" data-cat="all">All</button>` + cats.map(c=>{
      ...
      const active = state.selectedCategory !== 'all' && (selectedLabels.includes(categoryKey_(c.CategoryID)) || selectedLabels.includes(categoryKey_(c.Name)));
      return `<button class="tab ${active?'active':''}" data-cat="${esc(id)}">${esc(c.Name || c.CategoryName || id)}</button>`;
    }).join('');
    box.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{
      state.selectedCategory=resolveCategoryId_(b.dataset.cat);
      state.selectedSubCategory='all';
      localStorage.setItem('freshlySelectedCategory',state.selectedCategory);
      localStorage.setItem('freshlySelectedSubCategory','all');
      renderCategories();
  ...
  function renderProducts(){
    ...
    const subcategoryFilter = hasSpecificCategorySelected_() ? state.selectedSubCategory : 'all';
    const products = activeRows(state.products).filter(p => String(p.ApprovalStatus||'Approved').toLowerCase()==='approved' && String(p.WebsiteStatus||'Active').toLowerCase()==='active')
      ...
      .filter(p => productMatchesSubCategory_(p, subcategoryFilter))
      .filter(p => !term || `${p.Name} ${p.Description||''} ${p.CategoryID||''} ${p.SubCategoryID||''} ${p.SubCategory||''}`.toLowerCase().includes(term));
  ...
  function moneyAmount_(v){
    const raw = String(v ?? '').trim();
    if(!raw) return 0;
    const match = raw.replace(/,/g,'').match(/-?\d+(?:\.\d+)?/);
    return match ? Number(match[0]) || 0 : 0;
  }
  function optionName_(o){ return valueFrom_(o, ['OptionName','Option Name','Name','Title','Label']) || ''; }
  function optionType_(o){ return valueFrom_(o, ['OptionType','Option Type','Type']) || ''; }
  function optionProductId_(o){ return valueFrom_(o, ['ProductID','Product ID','ProductFreshlyID','Product Freshly ID']) || ''; }
  function optionCharge_(o){ return moneyAmount_(valueFrom_(o, ['AdditionalCharge','Additional Charge','Charge','ExtraCharge','Extra Charge','Price'])); }
  function isYes_(v){ return ['yes','y','true','1','active','available'].includes(String(v || '').trim().toLowerCase()); }
  function productOptionAvailable_(p, type){
    if(type === 'Cleaning') return isYes_(p.CleaningAvailable);
    if(type === 'Cutting') return isYes_(p.CuttingAvailable);
    if(type === 'Marination') return isYes_(p.MarinationAvailable);
    return false;
  }
  function fallbackOptionsFor_(p, type){
    if(!productOptionAvailable_(p, type)) return [];
    if(type === 'Cleaning') return [
      {OptionType:type, OptionName:'No Cleaning', AdditionalCharge:0},
      {OptionType:type, OptionName:'Cleaned', AdditionalCharge:20}
    ];
    if(type === 'Cutting'){
      const cat = String(p.CategoryID || p.SubCategory || p.Name || '').toLowerCase();
      const wholeLabel = cat.includes('fish') || String(p.Name||'').toLowerCase().includes('fish') ? 'Whole Fish' : 'As It Is';
      const cutLabel = cat.includes('fish') || String(p.Name||'').toLowerCase().includes('fish') ? 'Curry Cut' : 'Standard Cut';
      return [
        {OptionType:type, OptionName:wholeLabel, AdditionalCharge:0},
        {OptionType:type, OptionName:cutLabel, AdditionalCharge:30}
      ];
    }
    if(type === 'Marination') return [
      {OptionType:type, OptionName:'No Marination', AdditionalCharge:0},
      {OptionType:type, OptionName:'Basic Marination', AdditionalCharge:60},
      {OptionType:type, OptionName:'Kerala Fish Fry Masala Marination', AdditionalCharge:100},
      {OptionType:type, OptionName:'Kerala Fish Curry Masala Marination', AdditionalCharge:100}
    ];
    return [];
  }
  function productOptionsFor_(p, type){
    const pid = String(p.ProductID || p.ProductFreshlyID || '').trim();
    const rows = activeRows(state.productOptions || []).filter(o => String(optionProductId_(o)).trim() === pid && cleanKey_(optionType_(o)) === cleanKey_(type));
    const mapped = rows.map(o => ({
      OptionID: valueFrom_(o, ['OptionID','Option ID']) || '',
      OptionType: type,
      OptionName: optionName_(o),
      AdditionalCharge: optionCharge_(o),
      Status: valueFrom_(o, ['Status']) || 'Active'
    })).filter(o => o.OptionName);
    return mapped.length ? mapped : fallbackOptionsFor_(p, type);
  }
  function optionValue_(o){
    const name = String(o.OptionName || '').trim();
    const charge = Number(o.AdditionalCharge || 0);
    return charge > 0 ? `${name} (+₹${num(charge)})` : name;
  }
  function isNoOption_(text){ return /^no\s+/i.test(String(text || '').trim()) || String(text || '').trim().toLowerCase() === 'none'; }
  function optionChargesFromForm(fd){
    let extra=0; const notes=[];
    ['Cleaning','Cutting','Marination'].forEach(k=>{
      const v=fd.get(k);
      if(v && v!=='None'){
        const label = String(v).trim();
        const m=label.replace(/,/g,'').match(/\+₹\s*(\d+(?:\.\d+)?)/);
        if(m) extra += Number(m[1]) || 0;
        if(!isNoOption_(label) || (m && Number(m[1])>0)) notes.push(`${k}: ${label}`);
      }
    });
    const special=fd.get('SpecialNote');
    if(special) notes.push(`Note: ${special}`);
    return {extra, notes};
  }
  function productCard(p){
    ...
    return `<article class="product-card"><div class="product-img" data-open-product="${esc(p.ProductID)}">${img}</div><div class="product-body"><span class="stock ${stock.cls}">${esc(stock.label)}</span><h3 data-open-product="${esc(p.ProductID)}" style="cursor:pointer">${esc(p.Name)}</h3>${(p.SubCategory||p.SubCategoryID)?`<div class="product-subcategory">${esc(p.SubCategory || p.SubCategoryID)}</div>`:''}<div class="product-min-meta">${esc(p.PackSize || pr.priceBasis || 'Select quantity')}</div><div><span class="price">${priceLabel(pr)}</span> ${pr.mrp?`<span class="mrp">${currency}${num(pr.mrp)}</span>`:''}</div><div class="card-actions"><button class="btn btn-small" type="button" data-open-product="${esc(p.ProductID)}">Details</button><button class="btn btn-primary btn-small" type="button" data-open-product="${esc(p.ProductID)}">Select</button></div></div></article>`;
  }
  ...
  function optionSelectsHtml(p){
    return ['Cleaning','Cutting','Marination'].map(type=>{
      const options = productOptionsFor_(p, type);
      if(!options.length) return '';
      const selectOptions = options.map((o,idx)=>{
        const value = optionValue_(o);
        const charge = Number(o.AdditionalCharge || 0);
        const chargeText = charge > 0 ? ` + ${currency}${num(charge)}` : ' Free';
        return `<option value="${esc(value)}" data-charge="${esc(charge)}" ${idx===0?'selected':''}>${esc(o.OptionName)}${chargeText}</option>`;
      }).join('');
      return `<div class="product-option-field"><label>${esc(type)}</label><select class="input product-option-select" name="${esc(type)}">${selectOptions}</select></div>`;
    }).join('');
  }
  ...
  function renderProductDetailsPage(){
    ...
    const pr=effectivePrice(p); const stock=stockClass(p.StockStatus, pr.stockQty); const img=p.ImageURL?`<img src="${esc(p.ImageURL)}" alt="${esc(p.Name)}">`:`<span class="emoji">${categoryEmoji(p.CategoryID)}</span>`;
    box.innerHTML=`<div class="product-detail-grid standalone-detail"><div><div class="detail-image">${img}</div></div><div><span class="stock ${stock.cls}">${esc(stock.label)}</span><h2>${esc(p.Name)}</h2>${(p.SubCategory||p.SubCategoryID)?`<div class="product-subcategory detail-subcategory">${esc(p.SubCategory || p.SubCategoryID)}</div>`:''}<div class="product-min-meta">${esc(p.PackSize||pr.priceBasis||'Select quantity')}</div><div><span class="price">${priceLabel(pr)}</span> ${pr.mrp?`<span class="mrp">${currency}${num(pr.mrp)}</span>`:''}</div><p>${esc(p.Description||'Freshly sourced and quality checked.')}</p><div class="notice mini"><b>Freshness Note:</b> ${esc(p.FreshnessNote||'Freshly sourced and quality checked.')}</div><form id="detailSelectForm" class="detail-form"><div class="form-grid"><div><label>Choose Quantity</label><select class="input" name="SelectedQty">${quantityOptionsHtml(pr)}</select></div>${optionSelectsHtml(p)}<div class="full"><label>Special Note</label><input class="input" name="SpecialNote" placeholder="Optional preference or note"></div></div><div class="notice line-summary" data-line-summary></div><div class="card-actions"><button class="btn" type="button" onclick="window.close()">Close Window</button><button class="btn btn-primary" type="submit">Add to Cart</button></div></form></div></div>`;
  ...
  function normalizeOrderItemForSubmit_(x){
    return {
      ...
      SelectedOptions: x.SelectedOptions || x.Note || '',
      Note: x.Note || ''
    };
  }
