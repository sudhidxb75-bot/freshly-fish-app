...
function normalizeProduct(row, index) {
  const name = pick(row, ["Name", "ProductName", "Product Name", "Item", "ItemName", "Item Name", "Fish", "FishName", "Fish Name"]);
...
async function loadFish() {
  ...
  try {
    ...
    if (!PRODUCTS.length) {
      console.warn("Freshly product rows loaded, but none were visible. First rows:", normalized.slice(0, 5));
      container.innerHTML = `<div class="product-empty">Product CSV loaded, but no active products were found. Check <b>Name</b>, <b>Stock</b>, and <b>Status</b> columns.</div>`;
