...
const FRESHLY_CONFIG = {
  PRODUCT_CSV_URL: "PASTE_FISHDATA_OR_PRODUCTDATA_CSV_URL_HERE",
  HUB_PINCODES_CSV_URL: "PASTE_HUBPINCODES_CSV_URL_HERE",
  HUB_SLOTS_CSV_URL: "PASTE_HUBDELIVERYSLOTS_CSV_URL_HERE",
  BACKEND_WEB_APP_URL: "PASTE_APPS_SCRIPT_WEB_APP_URL_HERE",
...
function normalizeProduct(row, index) {
  const name = pick(row, ["Name", "ProductName", "Item", "ItemName"]);
  const category = pick(row, ["Category", "ProductCategory"], FRESHLY_CONFIG.DEFAULT_CATEGORY);
  const unit = pick(row, ["Unit", "SaleUnit", "UOM"], "kg");
  const status = pick(row, ["Status", "Active"], "Active");
  const stock = pick(row, ["Stock", "Availability"], "Available");

  return {
    id: pick(row, ["ProductID", "ID", "Code", "SKU"], name || `PRODUCT_${index + 1}`),
    category,
    subCategory: pick(row, ["SubCategory", "Sub Category"], ""),
    name,
    malayalam: pick(row, ["Malayalam", "MalayalamName", "MlName"], ""),
    price: money(pick(row, ["PricePerKg", "Price", "Rate", "UnitPrice"], 0)),
    unit,
    stock,
    image: pick(row, ["Image", "ImageURL", "Photo", "ImageUrl"], ""),
    cleanCharge: money(pick(row, ["CleanChargePerKg", "CleaningCharge", "CleanCharge"], 0)),
    curryCutCharge: money(pick(row, ["CurryCutChargePerKg", "CutCharge", "CuttingCharge"], 0)),
    steakCutCharge: money(pick(row, ["SteakCutChargePerKg"], 0)),
    filletCharge: money(pick(row, ["FilletChargePerKg"], 0)),
    marinationCharge: money(pick(row, ["MarinationChargePerKg", "MarinationCharge"], 0)),
    status,
    sortOrder: money(pick(row, ["SortOrder", "Sort"], index + 1)),
...
function isProductVisible(product) {
  ...
  if (["inactive", "disabled", "no", "hidden"].includes(status)) return false;
  if (["out", "out of stock", "not available", "na", "no"].includes(stock)) return false;
  return true;
}
...
async function loadFish() {
  ...
  try {
    ...
    PRODUCTS = rows.map(normalizeProduct).filter(isProductVisible).sort((a, b) => a.sortOrder - b.sortOrder || a.name.localeCompare(b.name));
    renderCategoryFilters();
    renderProducts();
  } catch (error) {
    console.error(error);
    container.innerHTML = `<div class="product-empty">Products could not be loaded. Check FishData CSV publish link and sheet headers.</div>`;
  }
}
