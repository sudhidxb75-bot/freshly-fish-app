/* Freshly v10 App Settings - Pincode Hub Fix */
...
async function updateHubPreview() {
  ...
  matchedHubs.forEach((hub, index) => {
    ...
    option.textContent = `${hub.hubName} - ${hub.area || hub.city} - Cutoff ${hub.cutOffTime || "Not set"}`;
...
function renderSelectedHub() {
  ...
  preview.innerHTML = `<b>Selected Freshly Hub:</b><br>
    ...
    Default Cut-off Time: ${selectedHub.cutOffTime || "Not set"}<br>
  ...
  if (fulfillment) {
    ...
    if (fulfillment.value === "Home Delivery" && !selectedHub.homeDeliveryAvailable) {
      fulfillment.value = "Hub Pickup";
    }

    if (fulfillment.value === "Hub Pickup" && !selectedHub.pickupAvailable) {
      fulfillment.value = "Home Delivery";
    }
  }
...
function getFulfillmentCharge() {
  ...
  if (fulfillment === "Home Delivery" && selectedHub) {
    return selectedHub.homeDeliveryCharge || 0;
  }
...
function toggleLang() {
  ...
  if (byId("title")) {
    byId("title").innerText = ml ? "ഇന്നത്തെ മീൻ ലഭ്യത" : "Today's Fresh Catch";
  }

  if (byId("cartTitle")) {
    byId("cartTitle").innerText = ml ? "നിങ്ങളുടെ കാർട്ട്" : "Your Cart";
  }
...
async function initFreshlyApp() {
  const savedAddress = localStorage.getItem("freshlyAddress");

  if (savedAddress && byId("address")) {
    byId("address").value = savedAddress;
  }

  if (byId("pincode")) {
    byId("pincode").addEventListener("input", updateHubPreview);
  }

  if (byId("city")) {
    byId("city").addEventListener("input", updateHubPreview);
  }

  if (byId("hubSelect")) {
    byId("hubSelect").addEventListener("change", selectHubFromDropdown);
  }

  if (byId("fulfillment")) {
    byId("fulfillment").addEventListener("change", function () {
      updateSlotDropdown();
      updateCart();
    });
  }
...
window.addEventListener("DOMContentLoaded", initFreshlyApp);
