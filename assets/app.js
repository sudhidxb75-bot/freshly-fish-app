(function(){
  // Freshly V3.8.9: backend-controlled banners + clean category/subcategory display.
  ...
  async function loadPublicData(){
    try{
      ...
      ['categories','subcategories','products','districts','hubs','areas','slots','districtPricing','banners','countries'].forEach(k => state[k] = p[k] || demo[k] || []);
  ...
  function subcategoryId_(s){ return valueFrom_(s, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','ID','Code']) || valueFrom_(s, ['Name','SubCategoryName','Sub Category Name','SubCategory','Sub Category','Title']); }
  function subcategoryName_(s){ return valueFrom_(s, ['Name','SubCategoryName','Sub Category Name','SubCategory','Sub Category','Title']) || valueFrom_(s, ['SubCategoryID','SubCategory ID','SubCategoryId','Sub Category ID','ID','Code']); }
  function subcategoryCategory_(s){ return valueFrom_(s, ['CategoryID','Category ID','ParentCategoryID','Parent Category ID','Category','MainCategory','Main Category']); }
  ...
  function getAvailableSubcategories_(){
    ...
    activeRows(state.subcategories).forEach(s => {
      if(!subcategoryBelongsToCategory_(s, state.selectedCategory)) return;
      add(subcategoryId_(s), subcategoryName_(s), subcategoryCategory_(s), s.SortOrder);
    });
    activeRows(state.products).forEach(p => {
      if(!productMatchesCategory_(p, state.selectedCategory)) return;
      const vals = productSubcategoryValues_(p);
      if(vals.length) add(vals[0], vals[1] || vals[0], p.CategoryID || p.Category || '', 999);
    });
