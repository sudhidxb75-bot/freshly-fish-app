(function(){
  ...
  function normalizeBanner_(b, source, idx){
    const image = normalizeBannerImageUrl_(bannerImageUrl_(b));
    return Object.assign({}, b, {
      ImageURL: image,
      MobileImageURL: normalizeBannerImageUrl_(bannerValue_(b, ['MobileImageURL','Mobile Image URL','MobileImage']) || image),
  ...
  function bindPromoSlider(){
    ...
    let i = 0;
    const go = n => { i = n % total; track.style.transform = `translateX(-${i*100}%)`; dots.forEach((d,idx)=>d.classList.toggle('active', idx===i)); };
  ...
  function checkPincodeHubs(){
    ...
    if(hubs.length === 1 && !state.selectedHub){
      state.selectedHub = hubs[0].HubID || '';
      renderHubDropdown('#hubSelect');
    }
    if(box){
  ...
  function matchingHubs(){
    ...
    return activeRows(state.hubs).filter(h => {
      const countryOk = !state.selectedCountry || (!h.CountryID && !h.Country) || String(h.CountryID || h.Country)===String(state.selectedCountry);
      const districtOk = !state.selectedDistrict || h.DistrictID===state.selectedDistrict || String(h.DistrictName || h.City || '').toLowerCase()===String(state.selectedDistrict).toLowerCase();
      const hubPin = String(h.Pincode || '').trim().toLowerCase();
      const area = String(h.Area || h.AreaName || h.Location || '').trim().toLowerCase();
      const pinOk = !pin || hubPin === pin || area === pin || area.includes(pin) || pin.includes(area);
  ...
  function renderHubDropdown(sel){
    document.querySelectorAll(sel).forEach(el=>{
      const hubs = matchingHubs();
      if(state.selectedHub && !hubs.some(h => h.HubID === state.selectedHub)) state.selectedHub = '';
      el.innerHTML = `<option value="">Choose Freshly Hub</option>` + hubs.map(h=>`<option value="${esc(h.HubID)}" ${h.HubID===state.selectedHub?'selected':''}>${esc(h.HubName)} - ${esc(h.Area || h.Pincode || '')}</option>`).join('');
      el.onchange=()=>{state.selectedHub=el.value;saveLocation();updateLocationToggleLabel_();renderCheckoutLocationControls();};
    });
  }
  function selectedHubRow_(){ return activeRows(state.hubs).find(h => h.HubID === state.selectedHub) || null; }
  ...
  function confirmFreshlyLocation(){
    ...
    if(!state.selectedHub && hubs.length === 1) state.selectedHub = hubs[0].HubID || '';
