(function(){
  ...
  function renderPromoSlider(){
    ...
    const backendBanners = Array.isArray(state.banners) ? state.banners : [];
    let banners = (backendBanners.length ? backendBanners : (demo.banners || []))
  ...
  function promoSlide(b, idx){
    ...
    if(image && displayMode === 'imageonly'){
      ...
      return `<article class="promo-slide backend-banner image-only"><a class="banner-image-link" href="${btnLink}" aria-label="${title}"><img src="${esc(image)}" alt="${title}"></a>${imageButton}</article>`;
    }
    ...
    return `<article class="promo-slide backend-banner ${image?'has-image':''}"${imgStyle}><div class="banner-copy"><span class="eyebrow">${label}</span><h2>${title}</h2><p>${sub}</p></div><a class="btn btn-primary banner-main-btn" href="${btnLink}">${btnText}</a></article>`;
  }
