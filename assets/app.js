/* Freshly v45 Full Package App Settings */
const CATEGORIES_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRUM5xUYa0T5smTpkKSH2tEYrD1RV1SHuiIdFIbJbIcHOM8AMzW9VjCIajP4gsyHfP2fVBOKX6zlDpV/pub?gid=797874223&single=true&output=csv";
const AREA_DEVELOPMENT_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRUM5xUYa0T5smTpkKSH2tEYrD1RV1SHuiIdFIbJbIcHOM8AMzW9VjCIajP4gsyHfP2fVBOKX6zlDpV/pub?gid=2031826112&single=true&output=csv";
const PRODUCTS_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRUM5xUYa0T5smTpkKSH2tEYrD1RV1SHuiIdFIbJbIcHOM8AMzW9VjCIajP4gsyHfP2fVBOKX6zlDpV/pub?gid=441866780&single=true&output=csv";
const FISH_DATA_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQ4tubmP5rpzNoYdlb_9v39MFLZEr0brxTjiwmg--F4eVzw6iiMWLXM9ZkpVvYxSapFHQcwkc0HmcUO/pub?gid=981806902&single=true&output=csv";
const HUB_PINCODES_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRUM5xUYa0T5smTpkKSH2tEYrD1RV1SHuiIdFIbJbIcHOM8AMzW9VjCIajP4gsyHfP2fVBOKX6zlDpV/pub?gid=887522093&single=true&output=csv";
const HUB_DELIVERY_SLOTS_URL = "https://docs.google.com/spreadsheets/d/e/2PACX-1vRUM5xUYa0T5smTpkKSH2tEYrD1RV1SHuiIdFIbJbIcHOM8AMzW9VjCIajP4gsyHfP2fVBOKX6zlDpV/pub?gid=1092283679&single=true&output=csv";
...
function areaRowsToObjects(csv){
  ...
  return rows.slice(1).map(cols=>{
    ...
    return {
      ...
      displayOrder:num(pick(r,['DisplayOrder','Display Order','SortOrder','Sort Order'])),
      notes:pick(r,['Notes'])
    };
  })
  .filter(x=>x.area||x.pincode||x.city||x.district)
...
function saveSelectedArea(area){
  ...
  selectedDistrict=selectedArea ? (selectedArea.district||selectedArea.city||'') : '';
...
function getDistricts(){
  const seen={};
  areaRows.forEach(a=>{
    const d=String(a.district||a.city||'').trim();
    if(d) seen[d]=true;
  });
...
function selectDistrict(district){
  selectedDistrict=String(district||'').trim();
  console.log('District selected:', selectedDistrict);
  selectedArea=null;
  localStorage.removeItem('freshlySelectedArea');
...
function updateTopLocationText(){
  const topLocationText = document.getElementById('topLocationText');
  if(!topLocationText) return;

  if(selectedArea){
    topLocationText.textContent = selectedArea.area || selectedArea.city || selectedArea.pincode || 'Kozhikode';
  }else if(selectedDistrict){
    topLocationText.textContent = selectedDistrict;
  }else{
    topLocationText.textContent = 'Select Location';
  }
}

function renderLocationSelector(){
  updateTopLocationText();
  ...
  // v34/v35 dropdown may not have locationButton. It only has topLocationText.
  if(!list) return;

  if(!selectedArea && !selectedDistrict) loadSelectedArea();

  if(btn){
    btn.innerHTML=selectedArea ? `📍 ${selectedArea.area||selectedArea.city||selectedArea.pincode}` : (selectedDistrict ? `📍 ${selectedDistrict}` : '📍 Select Location');
  }

  if(topLocationText){
    topLocationText.textContent = selectedArea ? (selectedArea.area||selectedArea.city||selectedArea.pincode||'Kozhikode') : (selectedDistrict || 'Select Location');
  }

  let html='';

  if(!areaRows.length){
    html='<p class="location-help">Area list not configured yet. Check AreaDevelopment CSV URL and published sheet.</p>';
    list.innerHTML=html;
    if(status){
      status.innerHTML='Area list not configured yet.';
  ...
  if(!selectedDistrict){
    ...
    html += '<div class="location-step-title">Select District</div>';

    districts.forEach(d=>{
      const activeCount=areaRows.filter(a=>String(a.district||a.city||'').trim()===d && isAreaActive(a)).length;
      html+=`<button class="location-option district-option" onclick="selectDistrict('${String(d).replace(/'/g,"\\'")}')"><b>${d}</b><span>${activeCount ? activeCount+' active area(s)' : 'View areas'}</span><small>Choose district</small></button>`;
    });

    list.innerHTML=html;

    if(status){
      status.innerHTML='Select your district first.';
  ...
  const districtAreas=areaRows.filter(a=>String(a.district||a.city||'').trim()===String(selectedDistrict).trim());

  html += `<button class="location-back" onclick="backToDistricts()">← Change District</button>`;
  html += `<div class="location-step-title">Select Area in ${selectedDistrict}</div>`;

  districtAreas.forEach((a)=>{
    const originalIndex=areaRows.indexOf(a);
    html+=`<button class="location-option" onclick="selectAreaByIndex(${originalIndex})"><b>${a.area||a.city||a.pincode}</b><span>${a.city||''} ${a.pincode?'- '+a.pincode:''}</span><small>${areaStatusText(a)}</small></button>`;
  });

  if(!districtAreas.length){
    html += '<p class="location-help">No areas added under this district yet.</p>';
  }
  ...
  if(status){
    if(!selectedArea){
      status.innerHTML=`Select your area in ${selectedDistrict}.`;
      status.className='area-status-box';
...
function productRowsToObjects(csv){
  const rows=parseCSV(String(csv||'').trim());
  if(rows.length<2) return [];
  const headers=rows[0];
  return rows.slice(1).map(cols=>{
    ...
    return {
      ...
      name:pick(r,['Name','ProductName','Product Name','FishName','Fish Name','Product','Item']),
      malayalam:pick(r,['Malayalam','MalayalamName','Malayalam Name','LocalName','Local Name']),
      price:num(pick(r,['Price','PricePerKg','Price Per Kg','Rate','KgRate','KG Rate'])),
      unit:pick(r,['Unit','UOM'])||'kg',
      minOrderQty:num(pick(r,['MinOrderQty','Min Order Qty','MinimumQty','Minimum Quantity']))||1,
      maxOrderQty:num(pick(r,['MaxOrderQty','Max Order Qty','MaximumQty','Maximum Quantity']))||5,
      incrementStep:num(pick(r,['IncrementStep','Increment Step','Step','QtyStep']))||1,
      stock:yes(pick(r,['Stock','InStock','In Stock','Available'])),
      productStatus:pick(r,['ProductStatus','Product Status','Status']),
      featured:yes(pick(r,['Featured'])),
      displayOrder:num(pick(r,['DisplayOrder','Display Order','SortOrder','Sort Order'])),
...
function fishRowsToObjects(csv){
  return productRowsToObjects(csv);
}
...
function hubRowsToObjects(csv){
  ...
  return rows.slice(1).map(cols=>{
    ...
    return {
      ...
      area:pick(r,['Area','Location','Locality']),
      pincode:pick(r,['Pincode','PinCode','PIN Code','PIN']),
      hubId:pick(r,['HubID','Hub ID']),
      hubName:pick(r,['HubName','Hub Name']),
      pickupAvailable:yes(pick(r,['PickupAvailable','Pickup Available'])),
      homeDeliveryAvailable:yes(pick(r,['HomeDeliveryAvailable','Home Delivery Available'])),
      homeDeliveryCharge:num(pick(r,['DeliveryCharge','HomeDeliveryCharge','Home Delivery Charge'])),
      cutOffTime:pick(r,['DefaultCutOffTime','Default CutOff Time','CutOffTime','Cut Off Time']),
      status:pick(r,['Status']),
      hubPartner:pick(r,['HubPartner','Hub Partner','PartnerName','Partner Name']),
      hubPhone:pick(r,['HubPhone','Hub Phone','Phone']),
      minimumOrder:num(pick(r,['MinimumOrder','Minimum Order'])),
      deliverySlot1:'',
      deliverySlot2:'',
      deliverySlot3:''
    };
...
function formatQtyLabel(qty, unit){
  const n = Number(qty);
  const cleanQty = Number.isInteger(n) ? String(n) : String(n);
  if(String(unit || '').toLowerCase() === 'kg' && n < 1){
    return Math.round(n * 1000) + ' gms';
  }
  return cleanQty + ' ' + (unit || 'unit');
}

function quantityOptions(product){
  const unit = product.unit || 'unit';
  let min = Number(product.minOrderQty || 1);
  let max = Number(product.maxOrderQty || min || 1);
  let step = Number(product.incrementStep || 1);
  if(!min || min <= 0) min = 1;
  if(!max || max < min) max = min;
  if(!step || step <= 0) step = 1;

  let html = '';
  for(let q = min; q <= max + 0.0001; q += step){
    const fixed = Math.round(q * 1000) / 1000;
    html += `<option value="${fixed}">${formatQtyLabel(fixed, unit)}</option>`;
  }
  return html;
}
  ...
  box.innerHTML = `
    <div class="details-card">
      ...
      <div class="details-content">
        <a class="back-link" href="index.html">← Back to Catalogue</a>
        <h2>${fish.name}</h2>
        ...
        <select id="detailWeight">
          ${quantityOptions(fish)}
        </select>

        <label><b>Cleaning / Cutting / Preparation</b></label>
...
function addSelectedFishToCart(){
  ...
  const productAmount = money(fish.price * weight);
  ...
  const amount = productAmount + cleaningCharge + marinationCharge;
...
async function loadFish(){
  ...
  const productUrl = (typeof PRODUCTS_URL !== 'undefined' && PRODUCTS_URL && !PRODUCTS_URL.includes('PASTE_')) ? PRODUCTS_URL : FISH_DATA_URL;
  if(!productUrl || productUrl.includes('PASTE_')){box.innerHTML='Product list is not configured yet.';return;}
  try{
    const csv=await fetchCsvNoCache(productUrl);
    fishData=productRowsToObjects(csv);
...
function addToCart(i){
  ...
  const marinationCharge = marination !== 'No Marination' ? money(fish.marinationChargePerKg * weight) : 0;
  const amount = productAmount + cleaningCharge + marinationCharge;
  cart.push({name: fish.name, malayalam: fish.malayalam, category: fish.category, price: fish.price, unit: fish.unit || 'kg', weight, cleaning, cleaningChargePerKg, cleaningCharge, marination, marinationChargePerKg: fish.marinationChargePerKg, marinationCharge, productAmount, amount});
  updateCart();
}
...
function updateCart(){
  ...
  cart.forEach((item,i)=>{
    subtotal += (item.productAmount || item.fishAmount || 0);
    ...
    list.innerHTML += `<div class="cart-item">
      ...
      Quantity: ${formatQtyLabel(item.weight, item.unit || 'kg')}<br>
      Rate: ₹${item.price}/${item.unit || 'kg'}<br>
      Product Amount: ₹${item.productAmount || item.fishAmount || 0}<br>
...
async function placeOrder(){
  ...
  const subtotal = cart.reduce((s,x)=>s+(x.productAmount || x.fishAmount || 0),0);
  ...
  const items = cart.map(x => `${x.name} - ${formatQtyLabel(x.weight, x.unit || 'kg')} - ${x.cleaning} ₹${x.cleaningCharge} - ${x.marination} ₹${x.marinationCharge} - Item Total ₹${x.amount}`).join(' | ');
...
🔴 END OF ORDER 🔴`;
  ...
  saveCart();
  updateCart();
  alert('Order submitted. WhatsApp order windows will open for configured numbers.\n\nThank you for supporting local entrepreneurs and community-based commerce through Freshly.');
}
...
window.onload = async function(){
  ...
  console.log('Area Rows Loaded:', areaRows);
...
const promoSlides = [
  {
    title: "Fresh Products Delivered Fresh Daily",
    text: "Fresh Fish • Meat • Fruits & Vegetables • Groceries",
    link: "index.html#fishContainer",
    cta: "Order Now"
  },
  {
    title: "Every Order Creates Local Opportunities",
    text: "Supporting Local Hub Partners, Delivery Partners, Suppliers and Small Businesses",
    link: "join-freshly.html",
    cta: "Join Freshly"
  },
  {
    title: "Your Area. Your Income.",
    text: "Become a Freshly Hub Partner and serve your community.",
...
function renderPromoSlider(){
  ...
  box.innerHTML = `
    <div class="promo-copy">
      ...
      <h2>${slide.title}</h2>
      <p>${slide.text}</p>
      <a href="${slide.link}">${slide.cta || "Register Now"}</a>
    </div>
  `;
}
...
// Location dropdown helper: close dropdown when user clicks outside
document.addEventListener('click', function freshlyLocationOutsideClose(event){
  const panel = document.getElementById('locationPanel');
  const trigger = event.target.closest('.fth-location, #locationButton, #locationPanel');
