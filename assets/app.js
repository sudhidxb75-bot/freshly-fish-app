(function(){
  ...
  function buildLocationOptions_(){
    const areas = activeRows(state.deliveryAreas || state.delivery || state.deliveryAreasData || []);
    const hubs = activeRows(state.localHubs || state.hubs || []);
    const districts = activeRows(state.districts || []);
    const opts = [];

    areas.forEach(a=>{
      const p = a.PinCode || a.Pincode || a.PIN || a.Zip || '';
      const area = a.Area || a.Location || a.City || a.Locality || '';
      const hub = a.HubName || a.Hub || '';
      const label = [area, p, hub].filter(Boolean).join(' - ');
      if(label) opts.push({label, pincode:p, city:a.City || a.District || '', area, hub:hub, raw:a});
    });

    hubs.forEach(h=>{
      const p = h.PinCode || h.Pincode || h.ServicePincodes || '';
      const hub = h.HubName || h.Name || h.LocalHubName || '';
      const label = [hub, h.Area || h.Location || '', p].filter(Boolean).join(' - ');
      if(label) opts.push({label, pincode:p, city:h.City || h.District || '', area:h.Area || h.Location || '', hub, raw:h});
    });

    districts.forEach(d=>{
      const label = d.DistrictName || d.Name || d.District || '';
      if(label) opts.push({label, pincode:d.DefaultPincode || d.PinCode || '', city:label, area:'', hub:'', raw:d});
    });

    const seen = new Set();
    return opts.filter(o=>{
      const k = String(o.label).toLowerCase();
  ...
  function renderLocationSelect_(){
    const boxes = document.querySelectorAll('[data-location-select-box]');
    if(!boxes.length) return;
    const opts = buildLocationOptions_();
    boxes.forEach(box=>{
      box.innerHTML = `<label class="field"><span>Select Location</span><select data-location-select><option value="">Choose your location / hub</option>${opts.map((o,i)=>`<option value="${i}">${esc(o.label)}</option>`).join('')}</select></label><button class="btn btn-primary" type="button" data-save-location>Set Location</button><p class="muted mini" data-location-selected></p>`;
      const select = box.querySelector('[data-location-select]');
      const saveBtn = box.querySelector('[data-save-location]');
      const selected = box.querySelector('[data-location-selected]');

      saveBtn.onclick = ()=>{
        const opt = opts[Number(select.value)];
        if(!opt){ toast('Please select your location.'); return; }
        const current = load('freshlySelectedLocation', {});
        const next = Object.assign({}, current, opt.raw || {}, {
          Label: opt.label,
          PinCode: opt.pincode,
          Pincode: opt.pincode,
          City: opt.city,
        ...
        save('freshlySelectedLocation', next);
        state.selectedLocation = next;

        ['#customerPincode','#checkoutPincode','#pincode','input[name="PinCode"]','input[name="Pincode"]'].forEach(sel=>{
          document.querySelectorAll(sel).forEach(el=>{ if(opt.pincode) el.value = opt.pincode; });
        });
        ['input[name="City"]','#city','#checkoutCity'].forEach(sel=>{
          document.querySelectorAll(sel).forEach(el=>{ if(opt.city) el.value = opt.city; });
        });
        if(selected) selected.textContent = `Selected: ${opt.label}`;
        toast('Location selected.');
        if(typeof renderLocationControls === 'function') renderLocationControls(); renderLocationSelect_();
        if(typeof renderCheckoutLocationControls === 'function') renderCheckoutLocationControls(); renderLocationSelect_();
      };

      const saved = load('freshlySelectedLocation', null);
      if(saved && saved.Label && selected) selected.textContent = `Selected: ${saved.Label}`;
    });
  }
  ...
  function bannerImageUrl_(b){
    return bannerValue_(b, [
      ...
      'DesktopImageURL','Desktop Image URL','Image','PhotoURL','Photo URL','PictureURL','Picture URL','Photo','Picture'
    ]);
  }
  ...
  function normalizeBanner_(b, source, idx){
    ...
    return Object.assign({}, b, {
      ImageURL: image,
      MobileImageURL: bannerValue_(b, ['MobileImageURL','Mobile Image URL','MobileImage']) || image,
  ...
  function renderPromoSlider(){
    ...
    const sheetRows = Array.isArray(state.banners) ? state.banners : [];
    const sheetBanners = normalizeBanners_(sheetRows, 'sheet');
    const demoBanners = normalizeBanners_(demo.banners || [], 'demo');

    let banners = sheetBanners.length ? sheetBanners : demoBanners;

    if(isMobileBannerView_()){
      const sheetImages = sheetBanners.filter(b => String(b.ImageURL || '').trim());
      const demoImages = demoBanners.filter(b => String(b.ImageURL || '').trim());
      banners = sheetImages.length ? sheetImages : (sheetBanners.length ? [] : demoImages);
    }else{
      banners = banners.filter(b => String(b.ImageURL || b.Title || '').trim());
    }

    banners = banners.sort((a,b)=>(+a.SortOrder||999)-(+b.SortOrder||999));
    ...
    track.dataset.bannerCount = String(Math.max(1,banners.length));
    track.innerHTML = banners.map((b,idx)=>promoSlide(b,idx)).join('');

    if(dotsBox){
      dotsBox.innerHTML = banners.length > 1
        ? banners.map((b,idx)=>`<button data-promo-dot="${idx}" class="${idx===0?'active':''}" aria-label="Promotion ${idx+1}"></button>`).join('')
        : '';
    }
  ...
  function promoSlide(b, idx){
    ...
    const label = esc(b.Label || b.Tag || 'Freshly');
    const btnText = esc(b.ButtonText || b.CTA || 'Shop Now');
    const btnLink = esc(b.ButtonLink || b.Link || '#shop');
    const displayMode = String(b.DisplayMode || '').toLowerCase();
    const hideTextOverlay = String(b.HideTextOverlay || b.ShowTextOverlay || '').toLowerCase();
    const wantsImageOnly = image && (displayMode === 'imageonly' || hideTextOverlay === 'yes' || hideTextOverlay === 'true');

    const desktopHeight = Math.max(180, Math.min(800, Number(b.DesktopHeight || b.BannerHeight || b.Height || 340)));
    const mobileHeight = Math.max(160, Math.min(520, Number(b.MobileHeight || b.MobileBannerHeight || 270)));
    const objectFit = String(b.ObjectFit || 'cover').trim() || 'cover';
    const mobileObjectFit = String(b.MobileObjectFit || b.MobileFit || 'cover').trim() || 'cover';
    const objectPosition = String(b.ObjectPosition || 'center center').trim() || 'center center';
      ...
      return `<article class="promo-slide backend-banner image-only" style="${cssVars}" data-banner-source="${esc(b._BannerSource || '')}">
        <a class="banner-image-link" href="${btnLink}" aria-label="${title}">
          <img data-banner-img src="${esc(image)}" data-fallback-src="${esc(b._FallbackImage || fallbackBannerImage_(idx))}" alt="${title}"
            onerror="if(!this.dataset.fallbackUsed){this.dataset.fallbackUsed='1';this.src=this.dataset.fallbackSrc;}else{this.closest('.promo-slide')?.classList.add('banner-image-error');}">
        </a>${imageButton}</article>`;
    }

    const bg = image ? `background-image:linear-gradient(90deg,rgba(255,255,255,.95),rgba(255,255,255,.68),rgba(255,255,255,.12)),url('${esc(image)}');` : '';
    return `<article class="promo-slide backend-banner ${image?'has-image':''}" style="${cssVars}${bg}">
      <div class="banner-copy"><span class="eyebrow">${label}</span><h2>${title}</h2><p>${sub}</p></div>
      <a class="btn btn-primary banner-main-btn" href="${btnLink}">${btnText}</a>
    </article>`;
  }
  ...
  function bindPromoSlider(){
    ...
    const slides = [...track.children].filter(el => el.classList && el.classList.contains('promo-slide') && !el.classList.contains('banner-image-error'));
    ...
    const show = n => {
      ...
      slides.forEach((slide, idx) => {
        ...
        const h = mobile
          ? (css.getPropertyValue('--banner-mobile-height').trim() || '270px')
          : (css.getPropertyValue('--banner-desktop-height').trim() || '340px');
        ...
        slide.querySelectorAll('img').forEach(img => {
          ...
          img.style.objectFit = mobile ? (css.getPropertyValue('--banner-mobile-object-fit').trim() || 'cover') : (css.getPropertyValue('--banner-object-fit').trim() || 'cover');
          img.style.objectPosition = mobile ? (css.getPropertyValue('--banner-mobile-object-position').trim() || 'center top') : (css.getPropertyValue('--banner-object-position').trim() || 'center center');
        });
      });
    ...
    dots.forEach((dot, idx) => { dot.onclick = () => show(idx); });

    if(window.freshlyPromoTimer) clearInterval(window.freshlyPromoTimer);
    show(0);
    if(total > 1) window.freshlyPromoTimer = setInterval(() => show(index + 1), 4200);
  }
  ...
  function freshlyCloseMobileMenus_(){
  ...
  function openShopCategory_(category, opts){
    const raw = String(category || 'all').trim() || 'all';
    if(opts && opts.fromMobileMenu) freshlyCloseMobileMenus_();
    const resolved = resolveCategoryId_(raw);
    state.selectedCategory = resolved || 'all';
    localStorage.setItem('freshlySelectedCategory', state.selectedCategory);

    if(!opts || opts.clearSearch !== false){
      ['#catalogSearch','#freshlyMobileSearch','#homeProductSearch'].forEach(sel=>{ const el=document.querySelector(sel); if(el) el.value=''; });
    }
    ...
    if(raw === 'Freshly Mart') localStorage.setItem('freshlyMartRequested','yes');
    if(opts && opts.fromMobileMenu) setTimeout(freshlyCloseMobileMenus_, 80);
  }

  window.freshlyOpenCategory = function(category, opts){
    openShopCategory_(category, Object.assign({clearSearch:true}, opts || {}));
  };
  ...
  function updateCustomerUI(){
    const name = state.customer ? (state.customer.Name || state.customer.CustomerName || state.customer.FullName || 'Customer') : '';
    ...
    document.querySelectorAll('[data-customer-state]').forEach(el=>{
      el.innerHTML = state.customer
        ? `<span class="login-badge">Logged in: ${esc(name)}${id ? ' ('+esc(id)+')' : ''}</span>`
        : `<span class="warning">Guest browsing. Login required only at checkout.</span>`;
    });
    ...
    const top = document.querySelector('[data-customer-top-login]');
    if(top){
      top.classList.toggle('logged-in', !!state.customer);
      top.innerHTML = state.customer
        ? `<span class="customer-icon">👤</span><span class="customer-top-name">${esc(name).split(' ')[0]}</span>`
        : `<span class="customer-icon">👤</span><span>Login</span>`;
      top.setAttribute('title', state.customer ? `Logged in as ${name}` : 'Customer Login / Signup');
    }
  }
