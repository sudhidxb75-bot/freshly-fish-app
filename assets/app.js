(function(){
  ...
  function init(){ setYear(); applyConfiguredContacts(); bindNav(); bindLocationDropdown(); window.freshlyDebugBannerCount = banners.length; bindPromoSlider(); bindGenericForms(); bindPortalForms(); bindCustomerAuthTabs(); bindTrackOrder(); bindCheckout(); bindCheckoutLocation(); bindAdmin(); if(document.querySelector('#shop') || document.querySelector('#productGrid')) loadPublicData(); updateCustomerUI(); updateCartUI(); }
  ...
  function renderPromoSlider(){
    ...
    // Sheet banners are preferred. Demo banners are used only if backend returns no Banners rows.
    let banners = sheetBanners.length ? sheetBanners : demoBanners;

    if(isMobileBannerView_()){
      const sheetImageBanners = sheetBanners.filter(b => String(b.ImageURL || '').trim());
      const demoImageBanners = demoBanners.filter(b => String(b.ImageURL || '').trim());
      // If the sheet has banner rows but no image URL, do not cover it with demo banners.
      banners = sheetImageBanners.length ? sheetImageBanners : (sheetBanners.length ? [] : demoImageBanners);
    }else{
    ...
    bindPromoSlider();
    setTimeout(()=>{ if(window.freshlyBannerCarouselFix) window.freshlyBannerCarouselFix(); }, 80);
    setTimeout(()=>{ if(window.freshlyMobileSafeLayout) window.freshlyMobileSafeLayout(); }, 100);
  }
  ...
  function bindPromoSlider(){
    ...
    const slides = [...track.children].filter(el => el.classList && el.classList.contains('promo-slide'));
    ...
    const go = n => {
      ...
      slides.forEach((s,idx)=>{
        ...
        const h = window.innerWidth <= 760
          ? (computed.getPropertyValue('--banner-mobile-height').trim() || '270px')
          : (computed.getPropertyValue('--banner-desktop-height').trim() || '340px');
        ...
        s.querySelectorAll('img').forEach(img=>{
          ...
          img.style.objectFit = window.innerWidth <= 760 ? 'cover' : (computed.getPropertyValue('--banner-object-fit').trim() || 'cover');
          img.style.objectPosition = window.innerWidth <= 760 ? 'center top' : (computed.getPropertyValue('--banner-object-position').trim() || 'center center');
        });
      });
    ...
    dots.forEach((d,idx)=>{
      if(d.dataset.boundPromoDot === 'yes') return;
      d.dataset.boundPromoDot = 'yes';
      d.addEventListener('click',()=>go(idx));
    });

    if(window.freshlyPromoTimer) clearInterval(window.freshlyPromoTimer);
    go(0);
    if(total > 1) window.freshlyPromoTimer = setInterval(()=>go(i+1), 4200);
